<template><div class="pg"><div class="hd glass-effect"><h2 class="t"><PictureOutlined :style="{color:'#f59e0b'}"/> AIGC 生成</h2></div><a-tabs v-model:activeKey="tab" class="glass-effect" style="padding:0 16px;border-radius:12px"><a-tab-pane key="text" tab="文本"/><a-tab-pane key="image" tab="图像"/><a-tab-pane key="audio" tab="音频"/></a-tabs><div class="body glass-effect"><div class="input-area"><a-textarea v-model:value="prompt" placeholder="输入提示词..." :rows="3"/><div class="params"><a-select v-model:value="model" style="width:180px" :options="['GPT-4','Claude-3','DeepSeek-V3'].map(v=>({label:v,value:v}))"/><a-button type="primary" :loading="gen" @click="generate">生成</a-button></div></div><div class="history" v-if="results.length"><h4>生成历史</h4><div class="result-grid"><div v-for="r in results" :key="r.id" class="r-card glass-effect"><div class="r-preview">{{ r.preview }}</div><div class="r-meta"><a-tag size="small">{{ r.model }}</a-tag><span class="r-time">{{ r.time }}</span></div></div></div></div></div></div></template>
