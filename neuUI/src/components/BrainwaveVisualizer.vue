<template>
  <div class="brainwave-visualizer">
    <canvas ref="canvasRef" class="brainwave-canvas" />
    <div class="brainwave-legend">
      <div v-for="wave in activeWaves" :key="wave.type" class="legend-item">
        <span class="legend-color" :style="{ background: wave.color }"></span>
        <span class="legend-label">{{ wave.label }}</span>
        <span class="legend-percent">{{ wave.percent }}%</span>
      </div>
    </div>
  </div>
</template>
