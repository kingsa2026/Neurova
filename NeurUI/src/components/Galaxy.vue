...
/**
 * Galaxy 银河星流背景（移植自 Vue Bits Galaxy，MIT）。
 * 针对 Neurova 的适配（相对上游源码）：
 * - 去 Tailwind：根容器 .nr-galaxy 用 scoped CSS 铺满父级并作背景层（pointer-events:none）；
 * - 因背景层不接收指针事件，鼠标交互监听从容器改挂 window/document，坐标按视口归一化；
 * - 鼠标平滑状态由 ref 改为普通对象，消除每帧响应式写入；
 * - 其余 shader 与 props API 保持上游一致（ogl 依赖）。
 */
...
// 普通对象（非 ref）：每帧 lerp 写入，避免无意义的响应式开销（上游用 ref 系缺陷）
...
const setup = () => {
  ...
  // 背景层 pointer-events:none，容器收不到指针事件 → 交互监听挂 window/document，
  // 坐标按视口尺寸归一（本组件按铺满页面的背景层使用，等价于原容器归一化）
...
<template>
  <div ref="ctnDom" class="nr-galaxy" v-bind="$attrs" aria-hidden="true" />
</template>
