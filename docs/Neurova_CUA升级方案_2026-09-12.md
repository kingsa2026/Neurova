...
> 交付状态（2026-09-12）：**Phase 0-2 全量 + R3-5 纪律段已落地**（前端已适配）；Phase 3 主体（R3-1/3/4）与远程会话平面（RS-1~5）各成立项书，见第 5/6 节交叉引用。
...
> 📌 **已独立立项（2026-09-12）**：R3-1/R3-3/R3-4 → `docs/Neurova_CUA_Phase3立项_2026-09-12.md`；R3-2 见第 6 节远程平面立项；**R3-5 已收口**（`context/rules_sections.py::build_computer_use_rules_section`，17/17 绿）。
...
| R3-5 CUA 使用纪律技能文档 | 随 R1 一起小幅先行 | ✅ **已完成（2026-09-12）**：`context/rules_sections.py::build_computer_use_rules_section` 写入系统提示恒定段——观察→语义→像素阶梯/反盲截图/快照过期/敏感禁区先问，双路径注入 + 17/17 测试绿 |

## 6. Phase 3 扩展 — 远程会话平面（RS-1~5，隔离桌面的自建轻量路线）

> 📌 **已独立立项（2026-09-12）**：RS-1~5 拆分为可独立验收批次 → `docs/Neurova_CUA_远程会话平面立项_2026-09-12.md`。
