...
4. 每阶段最长停留可配：phase_max_minutes_{light_sleep,deep_sleep,rem,hibernate}
   默认 30/60/120/240；dwell 超时强制向更深推进；hibernate 超时→回 active 并
   wake 记账。
...
class PhaseMaxSettingsTest(unittest.TestCase):
    """问题4：每阶段最长停留设置键（默认值 = 旧 B 设计 PhaseDurations 秒→分钟）"""

    def test_default_keys_and_values(self):
        sc, mm = _make_consolidation()
        s = sc.get_settings()
        self.assertEqual(s["phase_max_minutes_light_sleep"], 30)
        self.assertEqual(s["phase_max_minutes_deep_sleep"], 60)
        self.assertEqual(s["phase_max_minutes_rem"], 120)
        self.assertEqual(s["phase_max_minutes_hibernate"], 240)
...
class DwellProgressionTest(unittest.TestCase):
    ...
    def test_dwell_timeout_forces_next_phase(self):
        ...
        self.assertEqual(result, "deep_sleep", "阶段停留超时应强制向更深推进")
...
class AutoWakeDeadlineTest(unittest.TestCase):
    """问题5：手动 start_sleep 以 sleep_duration_minutes 登记唤醒 deadline"""
