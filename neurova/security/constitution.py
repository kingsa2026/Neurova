...
class ConstitutionEvaluationEngine:
    ...
    def evaluate(self, context: Dict[str, Any]) -> ConstitutionEvaluationResult:
        ...
        try:
            ...
            logger.info("Constitution evaluation: compliant=%s, score=%.2f, violations=%d, warnings=%d",
                       is_compliant, score, len(violations), len(warnings))
            ...
            return ConstitutionEvaluationResult(
                is_compliant=False,
                violations=[{"rule_id": "error", "message": str(e)}],
                warnings=[],
                score=0.0
            )
