...
class AuthSystem:
    ...
    def register_user(self, username: str, email: str, password: str, role: UserRole = UserRole.USER) -> Optional[User]:
        """
        注册用户
    ...
    def authenticate(self, username: str, password: str) -> Optional[Session]:
        ...
        try:
            ...
            if not result:
                logger.error("Password hash not found for user: %s", username)
                return None
    ...
    def _create_session(self, user: User, expires_in: int = 3600) -> Session:
        """
        创建会话
        ...
        session = Session(
            id=session_id, user_id=user.id, token=token, created_at=time.time(), expires_at=time.time() + expires_in
        )
    ...
    def validate_session(self, token: str) -> Optional[User]:
        ...
        try:
            ...
            logger.error("Failed to validate session: %s", e)
            return None
    ...
    def create_api_key(
        self, user_id: str, name: str, permissions: List[str] = None, expires_in: Optional[int] = None
    ) -> Optional[APIKey]:
        """
        创建 API 密钥
    ...
    def _update_api_key_last_used(self, key_id: str):
        ...
        try:
            ...
            logger.error("Failed to update API key last used: %s", e)
