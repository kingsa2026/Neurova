...
class ACPServer:
    ...
    def _register_routes(self) -> None:
        ...
        async def chat_endpoint(session_id: str, request: ACPChatRequest):
            return await self.chat_stream(session_id=session_id, message=request.message, stream=request.stream)
    ...
    async def create_session(
        self, user_id: str, model_id: str = "default", system_prompt: str = "", metadata: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """
        创建新会话
        ...
        with self._lock:
            ...
            # 创建会话配置
            session_config = ACPSessionConfig(system_prompt=system_prompt, metadata=metadata or {})
            ...
            # 添加系统消息
            if system_prompt:
                system_message = ACPMessage(role=ACPMessageRole.SYSTEM, content=system_prompt)
    ...
    async def chat_stream(self, session_id: str, message: str, stream: bool = True) -> Any:
        ...
        with self._lock:
            ...
            # 添加用户消息
            user_message = ACPMessage(role=ACPMessageRole.USER, content=message)
    ...
    async def _generate_stream_response(self, session_id: str, message: str):
        ...
        async def generate():
            ...
            yield ACPStreamChunk(
                event_type=ACPStreamEventType.THINKING, data={"step": "analyzing", "content": "正在分析您的问题..."}
            ).to_sse()
            ...
            for i in range(0, len(response_text), 10):
                ...
                yield ACPStreamChunk(event_type=ACPStreamEventType.TEXT_DELTA, data={"content": chunk}).to_sse()
                await asyncio.sleep(0.05)

            # 完成
            yield ACPStreamChunk(event_type=ACPStreamEventType.DONE, data={"message": "生成完成"}).to_sse()
    ...
    async def _generate_normal_response(self, session_id: str, message: str) -> Dict[str, Any]:
        ...
        with self._lock:
            session = self._sessions.get(session_id)
            if session:
                assistant_message = ACPMessage(role=ACPMessageRole.ASSISTANT, content=response_text)
                session.messages.append(assistant_message)

        return {"session_id": session_id, "message": response_text, "timestamp": datetime.datetime.now().timestamp()}
    ...
    async def switch_model(self, session_id: str, model_id: str) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            logger.info("会话 %s 切换模型: %s -> %s", session_id, old_model, new_model.model_name)
    ...
    async def update_session_config(self, session_id: str, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            logger.info("更新会话配置: %s", session_id)
            return {"success": True, "session_id": session_id}
...
# API 端点函数（用于直接调用）
async def create_session(
    user_id: str, model_id: str = "default", system_prompt: str = "", metadata: Dict[str, Any] = None
) -> Dict[str, Any]:
    """创建新 ACP 会话"""
