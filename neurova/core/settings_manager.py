...
class SettingsManager:
    ...
    def _load_settings(self) -> None:
        ...
        try:
            if self._settings_file.exists():
                ...
                logger.debug("从文件加载设置: %s", self._settings_file)
    ...
    def _save_settings_sync(self) -> None:
        ...
        try:
            ...
            logger.debug("设置已保存到: %s", self._settings_file)
        except Exception as e:
    ...
    def set_user_language(self, language: str, user_id: str = "default") -> bool:
        ...
        with self._lock:
            try:
                ...
                self._save_settings_sync()
                logger.info("用户 %s 语言设置为: %s", user_id, language)
                return True
            except Exception as e:
                logger.error("设置语言失败: %s", e)
                return False
    ...
    def set_user_timezone(self, timezone: str, user_id: str = "default") -> bool:
        ...
        with self._lock:
            try:
                ...
                self._save_settings_sync()
                logger.info("用户 %s 时区设置为: %s", user_id, timezone)
                return True
            except Exception as e:
                logger.error("设置时区失败: %s", e)
                return False
    ...
    def set_user_workspace(self, workspace: str, user_id: str = "default") -> bool:
        ...
        with self._lock:
            try:
                ...
                self._save_settings_sync()
                logger.info("用户 %s 工作空间设置为: %s", user_id, workspace)
                return True
            except Exception as e:
                logger.error("设置工作空间失败: %s", e)
                return False
    ...
    def update_settings(self, settings: Dict[str, Any], user_id: str = "default") -> bool:
        ...
        with self._lock:
            try:
                ...
                self._save_settings_sync()
                logger.info("用户 %s 设置已更新", user_id)
                return True
            except Exception as e:
                logger.error("更新设置失败: %s", e)
                return False
    ...
    def reset_settings(self, user_id: str = "default") -> bool:
        ...
        with self._lock:
            try:
                # 移除用户特定设置
                keys_to_remove = [k for k in self._settings.keys() if k.startswith(f"user_{user_id}_")]
                for key in keys_to_remove:
                    del self._settings[key]

                self._save_settings_sync()
                logger.info("用户 %s 设置已重置为默认值", user_id)
                return True
            except Exception as e:
