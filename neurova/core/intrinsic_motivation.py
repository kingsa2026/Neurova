...
class CompetenceDrive:
    ...
    def update_skill_level(self, success: bool, difficulty: float = 0.5) -> None:
        ...
        with self._lock:
            ...
            logger.debug("技能水平更新: %.3f", self.skill_level)
    ...
    def record_task(self, task_id: str, success: bool, difficulty: float = 0.5) -> None:
        ...
        """
        with self._lock:
            task_record = {"task_id": task_id, "success": success, "difficulty": difficulty, "timestamp": time.time()}
    ...
    def record_feedback(self, feedback: str, sentiment: float = 0.5) -> None:
        ...
        """
        with self._lock:
            feedback_record = {"feedback": feedback, "sentiment": sentiment, "timestamp": time.time()}
    ...
    def log_info(self) -> None:
        """记录日志信息"""
        logger.info(
            f"CompetenceDrive: skill={self.skill_level:.3f}, " f"completion_rate={self.task_completion_rate:.3f}"
        )
...
class AutonomyDrive:
    ...
    def record_choice(self, choice: str, satisfaction: float = 0.5) -> None:
        ...
        """
        with self._lock:
            choice_record = {"choice": choice, "satisfaction": satisfaction, "timestamp": time.time()}
    ...
    def log_info(self) -> None:
        """记录日志信息"""
        logger.info(
            f"AutonomyDrive: freedom={self.freedom_level:.3f}, "
...
class GrowthDrive:
    ...
    def calculate_intensity(self) -> float:
        ...
        with self._lock:
            ...
            if self.learning_history:
                recent_learning = len(
                    [h for h in self.learning_history[-10:] if time.time() - h["timestamp"] < 86400]
                )  # 最近24小时
    ...
    def record_learning(self, concept: str, understanding: float = 0.5) -> None:
        ...
        """
        with self._lock:
            learning_record = {"concept": concept, "understanding": understanding, "timestamp": time.time()}
    ...
    def calculate_growth_rate(self) -> float:
        ...
        with self._lock:
            ...
            # 计算最近学习频率
            recent_learning = len(
                [h for h in self.learning_history if time.time() - h["timestamp"] < 604800]
            )  # 最近一周
    ...
    def log_info(self) -> None:
        """记录日志信息"""
        logger.info(
            f"GrowthDrive: knowledge={self.knowledge_level:.3f}, "
...
class PurposeDrive:
    ...
    def calculate_intensity(self) -> float:
        ...
        with self._lock:
            ...
            if self.contributions:
                recent_contributions = len(
                    [c for c in self.contributions if time.time() - c["timestamp"] < 2592000]
                )  # 最近一个月
    ...
    def record_contribution(self, contribution: str, impact: float = 0.5) -> None:
        ...
        """
        with self._lock:
            contribution_record = {"contribution": contribution, "impact": impact, "timestamp": time.time()}
    ...
    def log_info(self) -> None:
        """记录日志信息"""
        logger.info(
            f"PurposeDrive: values={len(self.core_values)}, "
...
class IntrinsicMotivationSystem:
    ...
    def get_drive_state(self, drive_type: DriveType) -> DriveState:
        ...
        with self._lock:
            ...
            return DriveState(drive_type=drive_type, intensity=intensity, satisfaction=satisfaction)
    ...
    def update_drive_weights(self, weights: Dict[DriveType, float]) -> None:
        ...
        with self._lock:
            ...
            logger.debug("驱动权重更新: %s", self.drive_weights)
    ...
    def _on_action_executed(self, action: Action, success: bool) -> None:
        ...
        with self._lock:
            # 记录行动历史
            action_record = {"action": action.to_dict(), "success": success, "timestamp": time.time()}
