...
class Workspace:
    ...
    def set_manager(self, name: str, manager: Any) -> None:
        ...
        with self._lock:
            ...
            if setter:
                setter(manager)
                logger.info(f"Set manager '{name}' for workspace '{self._workspace_id}'")
            else:
                # 存储为自定义管理器
                self._reusable_services[name] = manager
                logger.info(f"Set custom manager '{name}' for workspace '{self._workspace_id}'")
    ...
    def start(self) -> bool:
        ...
        """
        if self._started:
            logger.warning(f"Workspace '{self._workspace_id}' already started")
            return True
        ...
        try:
            ...
            logger.info(f"Workspace '{self._workspace_id}' started")
            return True
            
        except Exception as e:
            logger.error(f"Failed to start workspace '{self._workspace_id}': {e}")
            return False
    ...
    def stop(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Workspace '{self._workspace_id}' stopped")
            return True
            
        except Exception as e:
            logger.error(f"Failed to stop workspace '{self._workspace_id}': {e}")
            return False
