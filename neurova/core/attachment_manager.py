...
class AttachmentManager:
    ...
    def _load_metadata(self) -> None:
        ...
        try:
            if self._metadata_file.exists():
                ...
                logger.debug("加载了 %s 个附件元数据", len(self._attachments))
        except Exception as e:
            logger.error("加载附件元数据失败: %s", e)
    ...
    def save(
        ...
        with self._lock:
            ...
            logger.info("附件已保存: %s - %s", attachment_id, filename)
    ...
    def delete(self, attachment_id: str) -> bool:
        ...
        with self._lock:
            if attachment_id not in self._attachments:
                logger.warning("附件不存在: %s", attachment_id)
                return False
            ...
            logger.info("附件已删除: %s", attachment_id)
            return True
    ...
    def _emit_attachment_event(self, event_type: str, attachment_info: AttachmentInfo) -> None:
        ...
        try:
            # 这里可以集成事件总线
            logger.debug("附件事件: %s - %s", event_type, attachment_info.attachment_id)
        except Exception as e:
            logger.error("发送附件事件失败: %s", e)
                ...
                if file_path.is_file() and file_path.name != "metadata.json":
                    ...
                    if file_id not in attachment_ids:
                        try:
                            file_path.unlink()
                            cleaned_count += 1
                            logger.debug("清理孤立文件: %s", file_path)
                        except Exception as e:
                            logger.error("清理孤立文件失败: %s", e)

            logger.info("清理了 %s 个孤立文件", cleaned_count)
