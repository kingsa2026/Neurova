...
class APIRouter:
        ...
        with self._lock:
            try:
                ...
                if method in self._endpoints[path]:
                    logger.warning(f"端点已存在，将覆盖: {method} {path}")
                ...
                logger.info(f"注册API端点: {method} {path} (插件: {plugin_name})")
                return True
        ...
        with self._lock:
            try:
                ...
                if method not in self._endpoints[path]:
                    logger.warning(f"方法不存在: {method} {path}")
                    return False
    ...
    def unregister_plugin_endpoints(self, plugin_name: str) -> int:
        ...
        with self._lock:
            ...
            logger.info(f"注销插件 {plugin_name} 的 {count} 个端点")
            return count
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "plugins": {
                    name: len(keys) for name, keys in self._plugin_endpoints.items()
                },
