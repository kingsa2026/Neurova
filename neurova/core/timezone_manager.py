...
class TimezoneManager:
        ...
        try:
            ...
            for zone_name in all_zones:
                try:
                    ...
                    logger.debug(f"加载时区失败 {zone_name}: {e}")
            
            # 加载常用时区
            self._common_timezones = self._get_common_timezone_list()
            
            logger.debug(f"加载了 {len(self._timezones)} 个时区")
            
        except Exception as e:
            logger.error(f"加载时区数据失败: {e}")
    ...
    def set_user_timezone(self, timezone_name: str, user_id: str = "default") -> bool:
        ...
        with self._lock:
            if not self.is_valid_timezone(timezone_name):
                logger.warning(f"无效的时区: {timezone_name}")
                return False
            
            self._user_timezones[user_id] = timezone_name
            logger.info(f"用户 {user_id} 时区设置为: {timezone_name}")
            return True
    
    def get_user_local_time(self, user_id: str = "default", 
                           utc_time: datetime.datetime = None) -> datetime.datetime:
        """
        获取用户本地时间
    ...
    def convert_time(self, time_value: datetime.datetime, 
                    from_timezone: str, to_timezone: str) -> datetime.datetime:
        """
        转换时间
    ...
    def format_time_for_user(self, time_value: datetime.datetime, 
                            user_id: str = "default",
    ...
    def search_timezones(self, query: str) -> List[TimezoneInfo]:
        ...
        with self._lock:
            ...
            for tz_info in self._timezones.values():
                # 搜索名称、偏移量和描述
                if (query_lower in tz_info.name.lower() or
                    query_lower in tz_info.offset.lower() or
                    query_lower in tz_info.description.lower()):
