...
def write_frontend_errors(errors: List[Dict[str, Any]], user_id: str = None,
    ...
    with _lock:
        try:
            ...
            logger.info(f"记录了 {len(errors)} 条前端错误日志")
            return True
            
        except Exception as e:
            logger.error(f"写入前端错误日志失败: {e}")
            return False


def read_all_errors(limit: int = 100, error_type: str = None,
...
def clear_errors(before_time: str = None, error_type: str = None) -> bool:
    ...
    with _lock:
        try:
            ...
            logger.info(f"清除了 {removed_count} 条错误日志")
            return True
            
        except Exception as e:
            logger.error(f"清除错误日志失败: {e}")
            return False
