...
from neurova.cognitive_layers.model_adapter.base import (
    AdapterCapabilities, BaseModelAdapter, ToolCall, ToolCallType
)
from neurova.cognitive_layers.model_adapter.registry import (
    _generate_with_litellm, _stream_with_litellm, get_model_adapter_registry
)
...
class DeepSeekAdapter(BaseModelAdapter):
    ...
    def format_prompt(self, messages: List[Dict[str, Any]], 
    ...
    async def generate_stream(self, prompt: Any, **kwargs) -> AsyncIterator[str]:
        ...
        try:
            ...
            self._last_error = str(e)
            self.logger.error(f"DeepSeek stream error: {e}")
            raise
...
class ClaudeAdapter(BaseModelAdapter):
    ...
    def _declare_capabilities(self) -> AdapterCapabilities:
        return AdapterCapabilities(
            ...
            custom_capabilities={
                "supports_extended_thinking": "claude-3.5" in self.model_name.lower() or "claude-3-opus" in self.model_name.lower(),
    ...
    async def generate(self, prompt: Any, **kwargs) -> str:
        ...
        try:
            ...
            self._last_error = str(e)
            self.logger.error(f"Claude generate error: {e}")
            raise
    ...
    async def generate_stream(self, prompt: Any, **kwargs) -> AsyncIterator[str]:
        ...
        try:
            ...
            self._last_error = str(e)
            self.logger.error(f"Claude stream error: {e}")
            raise
...
class HunYuanAdapter(BaseModelAdapter):
    ...
    def _declare_capabilities(self) -> AdapterCapabilities:
        return AdapterCapabilities(
            ...
            custom_capabilities={
                "supports_tencent_auth": True,
                "supports_regional_deployment": True
            }
        )
    ...
    async def generate(self, prompt: Any, **kwargs) -> str:
        ...
        try:
            ...
            self._last_error = str(e)
            self.logger.error(f"HunYuan generate error: {e}")
            raise
    ...
    async def generate_stream(self, prompt: Any, **kwargs) -> AsyncIterator[str]:
        ...
        try:
            ...
            self._last_error = str(e)
            self.logger.error(f"HunYuan stream error: {e}")
            raise
...
class GLMAdapter(BaseModelAdapter):
    ...
    def __init__(self, model_name: str, config: Optional[Dict[str, Any]] = None):
        super().__init__(model_name, config)
        self.api_key = config.get("api_key", "") if config else ""
        self.api_base = config.get("api_base", "https://open.bigmodel.cn/api/paas/v4") if config else "https://open.bigmodel.cn/api/paas/v4"
    ...
    async def generate(self, prompt: Any, **kwargs) -> str:
        ...
        try:
            ...
            self._last_error = str(e)
            self.logger.error(f"GLM generate error: {e}")
            raise
    ...
    async def generate_stream(self, prompt: Any, **kwargs) -> AsyncIterator[str]:
        ...
        try:
            ...
            self._last_error = str(e)
            self.logger.error(f"GLM stream error: {e}")
            raise
...
class KimiAdapter(BaseModelAdapter):
    ...
    async def generate(self, prompt: Any, **kwargs) -> str:
        ...
        try:
            ...
            self._last_error = str(e)
            self.logger.error(f"Kimi generate error: {e}")
            raise
    ...
    async def generate_stream(self, prompt: Any, **kwargs) -> AsyncIterator[str]:
        ...
        try:
            ...
            self._last_error = str(e)
            self.logger.error(f"Kimi stream error: {e}")
            raise
...
def register_builtin_adapters() -> None:
    ...
    for pattern, adapter_class, priority in adapter_mappings:
        registry.register_adapter_pattern(
            pattern=pattern,
            adapter_class=adapter_class,
            priority=priority
        )
        logger.info(f"Registered adapter pattern: {pattern} -> {adapter_class.__name__} (priority: {priority})")
