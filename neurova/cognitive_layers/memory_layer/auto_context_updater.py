...
class AutoContextUpdater:
    ...
    def _run_event_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                logger.error(f"AutoContextUpdater 更新失败: {e}")
    ...
    def _update_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                logger.error(f"AutoContextUpdater 更新循环失败: {e}")
                time.sleep(60)
    ...
    def _perform_update(self) -> None:
        ...
        with self._lock:
            ...
            try:
                ...
                logger.info(f"AutoContextUpdater 更新完成: 压缩 {compressed}, 温度更新 {temperature_updates}, "
    ...
    def _compress_old_memories(self) -> int:
        ...
        try:
            # 获取旧记忆
            threshold = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(days=self.compression_threshold_days)
            
            # 这里简化实现，实际应该调用记忆管理器的压缩方法
            logger.debug(f"压缩 {threshold} 之前的记忆")
            
            return 0
        except Exception as e:
            logger.error(f"压缩旧记忆失败: {e}")
            return 0
    ...
    def _update_temperature(self) -> int:
        ...
        try:
            # 更新所有记忆的温度（衰减）
            logger.debug(f"更新记忆温度，衰减率: {self.temperature_decay_rate}")
            
            return 0
        except Exception as e:
            logger.error(f"更新记忆温度失败: {e}")
            return 0
