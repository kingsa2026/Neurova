...
class RelationMixin:
    ...
    def create_relation(
        ...
        logger.debug(f"创建关联: {source_memory_id} -> {target_memory_id} ({relation_type})")
    ...
    def delete_relation(self, relation_id: str) -> bool:
        ...
        if source_id in self._memory_relations:
            self._memory_relations[source_id] = [
                rid for rid in self._memory_relations[source_id] if rid != relation_id
            ]
        
        if target_id in self._memory_relations:
            self._memory_relations[target_id] = [
                rid for rid in self._memory_relations[target_id] if rid != relation_id
            ]
        
        # 删除关联
        del self._relations[relation_id]
        
        logger.debug(f"删除关联: {relation_id}")
    ...
    def delete_memory_relations(self, memory_id: str) -> int:
        ...
        logger.debug(f"删除记忆 {memory_id} 的所有关联: {len(relation_ids)} 个")
