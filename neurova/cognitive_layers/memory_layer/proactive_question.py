...
class QuestionQueueManager:
    ...
    def add_question(self, question: Question) -> Question:
        ...
        logger.debug(f"添加问题: {question.id}")
        return question
    ...
    def mark_asked(self, question_id: str) -> Optional[Question]:
        ...
        logger.debug(f"标记问题为已提问: {question_id}")
        return question
    ...
    def mark_answered(self, question_id: str, answer: str) -> Optional[Question]:
        ...
        logger.debug(f"标记问题为已回答: {question_id}")
        return question
    ...
    def mark_dismissed(self, question_id: str) -> Optional[Question]:
        ...
        logger.debug(f"标记问题为已忽略: {question_id}")
        return question
    ...
    def remove_question(self, question_id: str) -> bool:
        ...
        logger.debug(f"移除问题: {question_id}")
        return True
    ...
    def clear_expired(self) -> int:
        ...
        if expired_ids:
            logger.debug(f"清理过期问题: {len(expired_ids)} 个")
...
class CuriosityDrive:
    ...
    def record_exploration(self, question_id: str, topic: str, curiosity_type: CuriosityType) -> ExplorationRecord:
        ...
        logger.debug(f"记录探索: {exploration.id}")
        return exploration
    
    def complete_exploration(self, exploration_id: str, findings: List[str], knowledge_gained: float = 0.0) -> Optional[ExplorationRecord]:
        """
        完成探索
        ...
        logger.debug(f"完成探索: {exploration_id}")
        return exploration
    ...
    def generate_curiosity_question(self, context: Dict[str, Any]) -> Optional[Question]:
        ...
        logger.debug(f"生成好奇心问题: {question.id}")
        return question
...
class ProactiveQuestionManager:
    ...
    def __init__(self, agent_id: str):
        ...
        logger.info(f"ProactiveQuestionManager 初始化: {agent_id}")
    
    def on_initialize(self) -> None:
        """初始化回调"""
        logger.debug(f"ProactiveQuestionManager 初始化完成: {self.agent_id}")
    
    def on_start(self) -> None:
        """启动回调"""
        self._session_active = True
        logger.debug(f"ProactiveQuestionManager 启动: {self.agent_id}")
    
    def on_stop(self) -> None:
        """停止回调"""
        self._session_active = False
        logger.debug(f"ProactiveQuestionManager 停止: {self.agent_id}")
    ...
    def create_question(
        ...
        logger.debug(f"创建问题: {question.id}")
        return question
    ...
    def reset_session(self) -> None:
        ...
        logger.debug(f"重置会话: {self.agent_id}")