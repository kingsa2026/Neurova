...
class MetaCognition:
    ...
    def __init__(
        ...
        logger.info(f"MetaCognition 初始化完成: agent_id={agent_id}")
    ...
    def update_state(
        ...
        with self._lock:
            ...
            load_score = self._calculate_load_score(
                active_tasks, memory_usage, response_time_ms, error_rate
            )
    ...
    def _analyze_load_trend(self) -> str:
        ...
        second_half = sum(s.load_score for s in recent_states[len(recent_states)//2:]) / (len(recent_states) - len(recent_states)//2)
