...
class TemporalPositionalEncoder:
    ...
    def __init__(self, num_frequencies: int = 10):
        self.absolute_encoder = PositionalEncoder(
            PositionalEncodingConfig(num_frequencies=num_frequencies)
        )
        self.relative_encoder = PositionalEncoder(
            PositionalEncodingConfig(num_frequencies=6)
        )
        self.periodic_encoder = PositionalEncoder(
            PositionalEncodingConfig(num_frequencies=4)
        )
    
    def encode_timestamp(self, timestamp: float,
                          reference_time: Optional[float] = None) -> List[float]:
        """
        编码时间戳 → 52维向量
    ...
    @property
    def output_dim(self) -> int:
        return (self.absolute_encoder.config.output_dim +
...
class EmotionPositionalEncoder:
    ...
    # 情感类型 → 默认效价和唤醒度
    DEFAULT_VALENCE = {"joy":.8,"trust":.7,"anticipation":.6,"surprise":.5,
                       "neutral":.5,"sadness":.2,"anger":.2,"fear":.2,"disgust":.1}
    DEFAULT_AROUSAL = {"anger":.9,"fear":.8,"surprise":.8,"joy":.7,
                       "anticipation":.6,"disgust":.5,"sadness":.3,"trust":.3,"neutral":.2}
    
    def __init__(self, num_frequencies: int = 6):
        self.intensity_enc = PositionalEncoder(
            PositionalEncodingConfig(num_frequencies=num_frequencies))
        self.valence_enc = PositionalEncoder(
            PositionalEncodingConfig(num_frequencies=4))
        self.arousal_enc = PositionalEncoder(
            PositionalEncodingConfig(num_frequencies=4))
    
    def encode_emotion(self, emotion_type: str, intensity: float,
                        valence: Optional[float] = None,
                        arousal: Optional[float] = None) -> List[float]:
        """
        编码情感状态 → 48维向量 (默认参数)
    ...
    @property
    def output_dim(self) -> int:
        return 9 + self.intensity_enc.config.output_dim + \
               self.valence_enc.config.output_dim + self.arousal_enc.config.output_dim
...
class ImportancePositionalEncoder:
    ...
    def __init__(self, num_frequencies: int = 4):
        self.value_enc = PositionalEncoder(
            PositionalEncodingConfig(num_frequencies=num_frequencies))
        self.rank_enc = PositionalEncoder(
            PositionalEncodingConfig(num_frequencies=3))
        self.stage_enc = PositionalEncoder(
            PositionalEncodingConfig(num_frequencies=2))
    
    def encode_importance(self, value: float,
                           rank_percentile: Optional[float] = None,
                           lifecycle_stage: Optional[str] = None) -> List[float]:
        """
        编码重要性 → 向量
        ...
        stage_map = {"active":1.0, "consolidated":.75, "archived":.5,
                     "forgotten":.25, "crystallized":.9}
    ...
    @property
    def output_dim(self) -> int:
        return (self.value_enc.config.output_dim +
                self.rank_enc.config.output_dim +
                self.stage_enc.config.output_dim)
