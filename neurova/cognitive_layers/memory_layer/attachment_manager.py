...
class AttachmentManager:
    ...
    def __init__(
        ...
        self._allowed_extensions = set(self._config.get("allowed_extensions", [
            '.txt', '.pdf', '.doc', '.docx', '.xls', '.xlsx', '.ppt', '.pptx',
            '.jpg', '.jpeg', '.png', '.gif', '.bmp', '.svg',
            '.mp3', '.wav', '.mp4', '.avi', '.mov',
            '.zip', '.rar', '.7z', '.tar', '.gz',
            '.json', '.xml', '.csv', '.md', '.html',
        ]))
        ...
        logger.info(f"AttachmentManager initialized: db={db_path}, storage={storage_dir}")
    ...
    def _init_attachment_table(self) -> None:
        ...
        try:
            ...
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_memory_attachments_memory_id ON memory_attachments (memory_id)")
            
            self._conn.commit()
            
        except Exception as e:
            logger.error(f"Failed to initialize attachment table: {e}")
            raise
        ...
        with self._lock:
            ...
            if self._conn:
                try:
                    ...
                    logger.error(f"Failed to save attachment to database: {e}")
                    # 清理已保存的文件
            ...
            logger.info(f"Attachment saved: {attachment_id} ({filename})")
    ...
    def get_attachment_data(self, attachment_id: str) -> Optional[bytes]:
        ...
        try:
            ...
            logger.error(f"Failed to read attachment data: {e}")
            return None
    ...
    def get_memory_attachments(self, memory_id: str) -> List[Dict[str, Any]]:
        ...
        with self._lock:
            ...
            try:
                ...
                logger.error(f"Failed to get memory attachments: {e}")
                return []
    ...
    def link_to_memory(self, attachment_id: str, memory_id: str) -> bool:
        ...
        with self._lock:
            ...
            try:
                ...
                logger.debug(f"Attachment {attachment_id} linked to memory {memory_id}")
                return True
                
            except Exception as e:
                logger.error(f"Failed to link attachment to memory: {e}")
                return False
    ...
    def unlink_from_memory(self, attachment_id: str, memory_id: str) -> bool:
        ...
        with self._lock:
            ...
            try:
                ...
                logger.debug(f"Attachment {attachment_id} unlinked from memory {memory_id}")
                return True
                
            except Exception as e:
                logger.error(f"Failed to unlink attachment from memory: {e}")
                return False
    ...
    def update_metadata(self, attachment_id: str, metadata: Dict[str, Any]) -> bool:
        ...
        with self._lock:
            ...
            try:
                ...
                logger.debug(f"Attachment metadata updated: {attachment_id}")
                return True
                
            except Exception as e:
                logger.error(f"Failed to update attachment metadata: {e}")
                return False
        ...
        with self._lock:
            ...
            if self._conn:
                try:
                    ...
                    logger.error(f"Failed to get file paths from database: {e}")
                ...
                for file in files:
                    ...
                    if file_path not in db_files:
                        try:
                            ...
                            result["errors"] += 1
                            logger.warning(f"Failed to clean orphaned file {file_path}: {e}")
            
            logger.info(f"Cleanup completed: {result['cleaned']} files cleaned, {result['freed_space']} bytes freed")
            return result
