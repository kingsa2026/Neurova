...
class AgentSelfManager:
    ...
    def __init__(self, agent_id: str, storage_path: Optional[str] = None):
        ...
        logger.info("AgentSelfManager 初始化: %s", agent_id)
    ...
    def initialize(self) -> None:
        ...
        logger.info("AgentSelfManager 初始化完成: %s", self.agent_id)
    ...
    def _load_commands(self) -> None:
        ...
        try:
            ...
            logger.debug("加载指令: %s", self.agent_id)
        except Exception as e:
            logger.error("加载指令失败: %s", e)
    ...
    def _save_commands(self) -> None:
        ...
        try:
            ...
            logger.debug("保存指令: %s 个", len(self._commands))
        except Exception as e:
            logger.error("保存指令失败: %s", e)

    def _load_heartbeat_tasks(self) -> None:
        """加载心跳任务"""
        try:
            # 从存储加载心跳任务
            logger.debug("加载心跳任务: %s", self.agent_id)
        except Exception as e:
            logger.error("加载心跳任务失败: %s", e)

    def _save_heartbeat_tasks(self) -> None:
        """保存心跳任务"""
        try:
            # 保存心跳任务到存储
            logger.debug("保存心跳任务: %s 个", len(self._heartbeat_tasks))
        except Exception as e:
            logger.error("保存心跳任务失败: %s", e)
    ...
    def add_command(self, command: CoreCommand) -> CoreCommand:
        ...
        logger.debug("添加指令: %s", command.name)
        return command
    ...
    def update_command(self, command_id: str, **kwargs) -> Optional[CoreCommand]:
        ...
        logger.debug("更新指令: %s", command.name)
        return command
    ...
    def delete_command(self, command_id: str) -> bool:
        ...
        if command_id in self._commands:
            ...
            self._save_commands()
            logger.debug("删除指令: %s", command_id)
            return True
        return False
    ...
    def add_heartbeat_task(self, task: HeartbeatTask) -> HeartbeatTask:
        ...
        if task.next_run_at is None:
            task.next_run_at = datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(
        ...
        logger.debug("添加心跳任务: %s", task.name)
        return task
    ...
    def update_heartbeat_task(self, task_id: str, **kwargs) -> Optional[HeartbeatTask]:
        ...
        logger.debug("更新心跳任务: %s", task.name)
        return task
    ...
    def record_task_run(self, task_id: str, success: bool, result: Optional[str] = None) -> Optional[HeartbeatTask]:
        ...
        logger.debug("记录任务运行: %s (成功: %s)", task.name, success)
        return task
