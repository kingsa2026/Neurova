...
class EnhancedMemoryRetriever:
        ...
        try:
            ...
            for memory in memories:
                ...
                total_score, score_breakdown = self._calculate_composite_score(
                    memory, context,
                    temperature_score, temporal_score,
                    emotional_score, importance_score,
                    activation_score,
                )
            ...
            logger.debug(f"检索完成: {len(results)} 条记忆, 耗时 {retrieval_time_ms:.2f}ms")
            return results
        except Exception as e:
    ...
    def decay_all_activations(self, current_time: Optional[datetime.datetime] = None) -> None:
        ...
        with self._lock:
            ...
            logger.debug(f"衰减完成: 移除 {len(to_remove)} 条低激活记忆")
