...
class MemoryCompressor:
    ...
    def _get_old_memories(
        ...
        try:
            ...
            logger.warning(f"Failed to get old memories: {e}")
            return []
        ...
        for category, group in category_groups.items():
            if len(group) <= 2:
                ...
                try:
                    ...
                    logger.warning(f"LLM compression failed for category {category}: {e}")
                    compressed_memories.extend(group)
...
4. 使用中文"""
            ...
            logger.warning(f"LLM merge summary generation failed: {e}")
            return None
    ...
    def _cleanup_compressed(self, original_ids: List[str]) -> None:
        ...
        for memory_id in original_ids:
            try:
                ...
                logger.warning(f"Failed to delete compressed memory {memory_id}: {e}")
