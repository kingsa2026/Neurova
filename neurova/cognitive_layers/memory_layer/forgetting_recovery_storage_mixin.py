...
class ForgettingRecoveryStorageMixin:
    ...
    def archive_memory(
        ...
        logger.debug(f"归档记忆: {memory_id} (原因: {reason})")
    ...
    def delete_memory(
        ...
        if permanent:
            # 永久删除，不保留恢复能力
            logger.debug(f"永久删除记忆: {memory_id}")
        else:
            # 软删除，保留恢复能力
            self._deleted_memories[memory_id] = delete_record
            logger.debug(f"软删除记忆: {memory_id}")
    ...
    def recover_memory(self, memory_id: str) -> Optional[Dict[str, Any]]:
        ...
        if memory_id in self._archived_memories:
            ...
            logger.debug(f"从归档恢复记忆: {memory_id}")
            return memory_data
        ...
        if memory_id in self._deleted_memories:
            ...
            logger.debug(f"从软删除恢复记忆: {memory_id}")
            return memory_data
    ...
    def cleanup_old_archives(self, days: int = 30) -> int:
        ...
        if to_delete:
            logger.debug(f"清理 {len(to_delete)} 个超过 {days} 天的归档记忆")
    ...
    def clear_all(self) -> Dict[str, int]:
        ...
        logger.debug(f"清空所有存储: 归档 {archived_count}, 删除 {deleted_count}, 历史 {history_count}")
