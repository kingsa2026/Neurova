...
class MemoryVersionControl:
        ...
        try:
            if hasattr(self._storage, 'fetch_all'):
                rows = self._storage.fetch_all(
                    "SELECT * FROM memory_versions ORDER BY timestamp DESC"
                )
                ...
                logger.info(f"加载了 {self._stats['total_versions']} 个版本")
        except Exception as e:
            logger.warning(f"加载版本失败: {e}")
    ...
    def create_snapshot(
        ...
        with self._lock:
            ...
            logger.debug(f"创建快照: {version_id} for memory {memory_id}")
            return version
