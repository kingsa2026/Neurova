...
class EmotionConductionModule:
    ...
    def get_positive_memories(self, threshold: float = 0.3, limit: int = 10) -> List[str]:
        """获取正面情感记忆"""
        with self._lock:
            positive = [(mid, val) for mid, val in self._emotion_values.items() if val > threshold]
    ...
    def get_negative_memories(self, threshold: float = -0.3, limit: int = 10) -> List[str]:
        """获取负面情感记忆"""
        with self._lock:
            negative = [(mid, val) for mid, val in self._emotion_values.items() if val < threshold]
    ...
    def get_neutral_memories(self, threshold: float = 0.2, limit: int = 10) -> List[str]:
        """获取中性情感记忆"""
        with self._lock:
            neutral = [(mid, val) for mid, val in self._emotion_values.items() if abs(val) <= threshold]
