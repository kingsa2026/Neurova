...
class SelfModelModule:
    ...
    def init(self) -> bool:
        ...
        logger.info(f"SelfModelModule initialized for agent '{self._agent_id}'")
        return True
    ...
    def assess_task(self, task_description: str, requirements: List[str]) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            # 计算平均置信度
            avg_confidence = (
                sum(match[2] for match in capability_matches) / len(capability_matches)
                if capability_matches else 0.5
            )
        ...
        with self._lock:
            return {
                ...
                "avg_capability_confidence": (
                    ...
                    if self._known_capabilities else 0
                ),
                "avg_expertise": (
                    sum(self._knowledge_domains.values()) / len(self._knowledge_domains)
                    if self._knowledge_domains else 0
                ),
            }
