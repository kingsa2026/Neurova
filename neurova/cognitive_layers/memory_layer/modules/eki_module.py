...
class EKIModule:
    ...
    def update_with_observation(
        ...
        with self._lock:
            ...
            # 4. 更新集合
            updated_ensemble = self._update_ensemble(
                ensemble, mean_state, observation, kalman_gain
            )
    ...
    def _generate_ensemble(self, mean_state: List[float]) -> List[List[float]]:
        ...
        for _ in range(self._ensemble_size):
            perturbed = [
                max(0.0, min(1.0, v + random.gauss(0, 0.05)))
                for v in mean_state
            ]
    ...
    def _update_ensemble(
        ...
        for state in ensemble:
            new_state = [
                state[i] + kalman_gain[i] * innovation
    ...
    def _inflate(
        ...
        for state in ensemble:
            new_state = [
                mean[i] + self._inflation_factor * (state[i] - mean[i])
                for i in range(len(state))
            ]
