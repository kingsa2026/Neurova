...
class AutoContextModule:
    ...
    def summarize_context(self) -> str:
        ...
        with self._lock:
            ...
            # 简单摘要：提取关键信息
            user_messages = [
                entry["content"] for entry in self._current_context
                if entry["role"] == "user"
            ]
        ...
        with self._lock:
            for entry in reversed(self._context_history):
                if entry.get("context_id") == context_id:
                    ...
                    self._context_length = sum(
                        len(e.get("content", "")) for e in self._current_context
                    )
                    return True
    ...
    def _compress_context(self) -> None:
        ...
        # 重新计算长度
        self._context_length = sum(
            len(entry.get("content", "")) for entry in self._current_context
        )
