...
class ToolHistoryTracker:
    ...
    def __init__(self, max_history: int = 10000, anomaly_threshold: float = 0.3):
        ...
        logger.info(f"ToolHistoryTracker initialized: max_history={max_history}")
    ...
    def record(self, entry: ToolExecutionEntry) -> None:
        ...
        logger.debug(f"Recorded tool execution: {entry.tool_name}")
    ...
    def get_recent(self, limit: int = 100, 
                   tool_name: Optional[str] = None) -> List[ToolExecutionEntry]:
        """
        获取最近的执行记录
    ...
    def get_by_tool(self, tool_name: str, 
                    limit: Optional[int] = None) -> List[ToolExecutionEntry]:
        """
        按工具名获取执行记录
    ...
    def get_failures(self, limit: int = 100, 
                     tool_name: Optional[str] = None) -> List[ToolExecutionEntry]:
        """
        获取失败记录
    ...
    def get_since(self, timestamp: float, 
        ...
        if tool_name:
            indices = self._by_tool.get(tool_name, [])
            entries = [self._history[i] for i in indices 
                      if i < len(self._history) and self._history[i].timestamp >= timestamp]
        else:
    ...
    def get_top_tools(self, limit: int = 10, 
                      sort_by: str = "total_calls") -> List[Dict[str, Any]]:
        """
        获取使用最多的工具
        ...
        for tool_name, stats in self._stats.items():
            ...
            if stats.total_calls >= 5 and stats.failure_rate > self.anomaly_threshold:
                anomaly = ToolAnomaly(
                    ...
                    description=f"High failure rate: {stats.failure_rate:.2%}",
                    context={
                        "total_calls": stats.total_calls,
            ...
            if len(recent_failures) >= 3:
                # 计算近期失败率
                recent_total = len([e for e in self._history 
                ...
                if recent_total > 0:
                    ...
                    if recent_failure_rate > self.anomaly_threshold * 1.5:
                        anomaly = ToolAnomaly(
                            ...
                            description=f"Recent failure spike: {recent_failure_rate:.2%} in last hour",
                            context={
    ...
    def find_tool_pairs(self, min_support: int = 3, 
    ...
    def _cleanup_old_entries(self) -> None:
        ...
        logger.info(f"Cleaned up {remove_count} old entries")
...
# 便捷函数
def create_tool_history_tracker(max_history: int = 10000, 
                               anomaly_threshold: float = 0.3) -> ToolHistoryTracker:
    """
    创建工具历史追踪器
