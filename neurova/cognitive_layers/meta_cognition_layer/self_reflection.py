            ...
            for emotion, count in emotion_counts.most_common(3):
                ...
                if ratio > 0.1:  # 超过10%的记忆有相同情感
                    pattern = MemoryPattern(
                        ...
                        description=f"情感 '{emotion}' 频繁出现，占比 {ratio:.1%}",
                        frequency=count,
            ...
            if hour_counts:
                ...
                if peak_ratio > 0.15:  # 超过15%的记忆在同一小时
                    pattern = MemoryPattern(
                        ...
                        description=f"在 {peak_hour[0]}:00 时段记忆最活跃，占比 {peak_ratio:.1%}",
            ...
            if day_counts:
                ...
                if day_ratio > 0.2:  # 超过20%的记忆在同一天
                    pattern = MemoryPattern(
                        ...
                        pattern_type=PatternType.TEMPORAL,
                        description=f"在 {peak_day[0]} 记忆最活跃，占比 {day_ratio:.1%}",
            ...
            for category, count in category_counts.most_common(3):
                ...
                if ratio > 0.2:  # 超过20%的记忆属于同一类别
                    pattern = MemoryPattern(
                        ...
                        description=f"类别 '{category}' 占主导地位，占比 {ratio:.1%}",
                        frequency=count,
                        confidence=min(1.0, ratio * 2),
                        metadata={
                            "category": category,
        ...
        try:
            ...
            if negative_ratio > 0.5:  # 超过50%的负面情感
                anomaly = Anomaly(
                    ...
                    description=f"负面情感比例过高: {negative_ratio:.1%}",
                    severity="high",
    ...
    def generate_insights(
        ...
        with self._lock:
            try:
                ...
                logger.info(f"生成 {len(insights)} 个洞察")
                return insights
            except Exception as e:
            ...
            for pattern in patterns:
                if pattern.confidence > 0.7:  # 高置信度模式
                    insight = Insight(
                        ...
                        description=f"检测到 {pattern.pattern_type.value} 类型的模式，置信度 {pattern.confidence:.1%}",
