...
class SkillsManager:
    ...
    def _load_skills(self) -> None:
        ...
        try:
            if hasattr(self._storage, 'fetch_all'):
                ...
                logger.info(f"加载了 {self._stats['total_skills']} 个技能")
        ...
        try:
            if isinstance(row, dict):
                ...
                return Skill(
                    ...
                    created_at=datetime.datetime.fromisoformat(row[8]) if row[8] else datetime.datetime.now(datetime.timezone.utc),
                    updated_at=datetime.datetime.fromisoformat(row[9]) if row[9] else datetime.datetime.now(datetime.timezone.utc),
    ...
    def _extract_keywords(self, text: str) -> List[str]:
        ...
        stop_words = {
            ...
            'which', 'who', 'whom', 'when', 'where', 'why', 'how', 'any', 'few',
    ...
    def auto_generate_skill(
        ...
        try:
            ...
            logger.info(f"自动生成技能: {skill.skill_id} - {name}")
            return skill
        except Exception as e:
            logger.error(f"自动生成技能失败: {e}")
            return None
        ...
        with self._lock:
            ...
            try:
                ...
                if fix_step:
                    ...
                    logger.info(f"技能已修补: {skill_id}")
                    return skill
                else:
                    logger.warning(f"无法生成修复步骤: {skill_id}")
                    return None
            except Exception as e:
                logger.error(f"自我修补失败: {e}")
                return None
    ...
    def _generate_fix_step(
        ...
        if self._llm_client:
            try:
                ...
                logger.warning(f"使用LLM生成修复步骤失败: {e}")
        ...
        with self._lock:
            ...
            return {
                **self._stats,
                ...
                "avg_success_rate": sum(s.success_rate for s in active_skills) / len(active_skills) if active_skills else 0.0,
            }
