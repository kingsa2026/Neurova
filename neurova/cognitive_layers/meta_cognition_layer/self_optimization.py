...
class SelfOptimization:
    ...
    def optimize_temperature(
        ...
        with self._lock:
            try:
                ...
                if self._temperature_engine:
                    for memory_id, new_temp in adjustments:
                        try:
                            ...
                            logger.warning(f"调整温度失败: {memory_id} - {e}")
                ...
                logger.info(f"温度优化完成: 调整了 {adjusted_count} 条记忆")
                return result
            except Exception as e:
                logger.error(f"温度优化失败: {e}")
    ...
    def prune_memories(
        ...
        with self._lock:
            try:
                ...
                logger.info(f"记忆修剪完成: 删除了 {pruned_count} 条记忆")
                return result
            except Exception as e:
    ...
    def restructure_associations(
        ...
        with self._lock:
            try:
                # 计算记忆之间的相似度
                associations = self._calculate_associations(
                    memories, similarity_threshold, max_associations_per_memory
                )
                ...
                if self._memory_manager:
                    for memory_id, related_ids in associations.items():
                        try:
                            ...
                            logger.warning(f"更新关联失败: {memory_id} - {e}")
                ...
                logger.info(f"关联重构完成: 更新了 {applied_count} 个记忆")
                return result
            except Exception as e:
                logger.error(f"关联重构失败: {e}")
