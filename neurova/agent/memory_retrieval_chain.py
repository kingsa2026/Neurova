...
class MemoryRetrievalChain:
    ...
    def add_retriever(self, retriever: Retriever) -> None:
        ...
        logger.debug(f"Added retriever: {retriever.name} (priority: {retriever.priority})")
    ...
    def remove_retriever(self, name: str) -> bool:
        ...
        for i, retriever in enumerate(self._retrievers):
            if retriever.name == name:
                ...
                logger.debug(f"Removed retriever: {name}")
                return True
        
        logger.warning(f"Retriever not found: {name}")
        return False
    ...
    async def retrieve(self, context: RetrievalContext) -> RetrievalResult:
        ...
        logger.info(f"Starting retrieval: query='{context.query[:50]}...', strategy={context.strategy.value}")
        ...
        try:
            ...
            if result.quality < context.min_quality:
                logger.warning(f"Retrieval quality {result.quality:.2f} < min_quality {context.min_quality:.2f}")
                # 尝试缓存降级
            ...
            logger.info(f"Retrieval completed: source={result.source}, quality={result.quality_level.value}, memories={len(result.memories)}")
    ...
    def clear_cache(self) -> int:
        ...
        self._cache.clear()
        logger.debug(f"Cleared cache: {count} entries")
        return count
    ...
    async def _retrieve_chain(self, context: RetrievalContext) -> RetrievalResult:
        ...
        for retriever in self._retrievers:
            try:
                logger.debug(f"Trying retriever: {retriever.name}")
                ...
                if result.memories and result.quality >= context.min_quality:
                    ...
                    logger.debug(f"Retriever {retriever.name} returned insufficient results")
    ...
    async def _retrieve_fallback(self, context: RetrievalContext) -> RetrievalResult:
        ...
        try:
            logger.debug(f"Trying primary retriever: {primary_retriever.name}")
            ...
            if result.memories and result.quality >= context.min_quality:
                ...
                logger.debug(f"Primary retriever {primary_retriever.name} failed quality check")
                
        except Exception as e:
            logger.warning(f"Primary retriever {primary_retriever.name} failed: {e}")
        
        # 降级到备用检索器
        for retriever in self._retrievers[1:]:
            try:
                logger.debug(f"Falling back to retriever: {retriever.name}")
                result = await retriever.retrieve(context)
                result.retrieval_time = asyncio.get_event_loop().time() - start_time
                ...
                logger.warning(f"Fallback retriever {retriever.name} failed: {e}")
                continue
    ...
    async def _retrieve_from_cache(self, context: RetrievalContext) -> Optional[RetrievalResult]:
        ...
        if cached:
            logger.debug(f"Cache hit for query: {context.query[:50]}...")
            return cached
