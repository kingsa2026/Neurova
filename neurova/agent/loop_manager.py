...
class LoopManager:
    ...
    def __init__(self, agent: Any):
        ...
        logger.debug(f"LoopManager initialized for agent: {getattr(agent.config, 'name', 'unknown')}")
    ...
    def initialize_sync(self) -> bool:
        ...
        try:
            ...
            if not self._check_loop_functionality(loop_instance):
                logger.warning(f"Loop {loop_class.__name__} has limited functionality")
            ...
            logger.error(f"Agent Loop initialization failed: {e}")
    ...
    async def rebuild(self, model_name: str) -> bool:
        ...
        if model_name == self._last_model and self._loop is not None:
            logger.debug(f"Same model {model_name}, no rebuild needed")
            return True
        ...
        try:
            ...
            if not self._check_loop_functionality(new_loop):
                logger.warning(f"New Loop {loop_class.__name__} has limited functionality")
                self._loop = new_loop
            ...
            logger.error(
                f"Agent Loop rebuild failed: {old_loop_name} → {loop_class_name or 'None'}: {e}"
            )
            # 保留旧 Loop
            self._loop = old_loop
            return False
