...
class InitializationManager:
        ...
        try:
            ...
            for dep_name in component_info["deps"]:
                if dep_name not in self._components:
                    raise RuntimeError(
                        f"Dependency '{dep_name}' of component '{name}' is not registered"
                    )
    ...
    def _topological_sort(self) -> List[str]:
        ...
        if len(result) != len(self._components):
            ...
            cycle_components = list(remaining)
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "components_with_deps": sum(
                1 for deps in self._dependencies.values() if deps
            ),
    ...
    def _max_dependency_depth(self) -> int:
        ...
        return max(
            (get_depth(name) for name in self._components),
            default=0
        )
