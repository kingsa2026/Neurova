...
class KeyframeToVideoGenerator(BaseGenerator):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super().__init__(
            ...
            generator_type=GeneratorType.KEYFRAME_TO_VIDEO,
            **kwargs
        )
        ...
        self.logger.info(f"KeyframeToVideoGenerator 初始化完成: provider={provider}")
    ...
    async def generate(self, config: GenerationConfig) -> GenerationResult:
        ...
        try:
            ...
            duration = time.time() - start_time
            self.logger.error(f"首尾帧生成视频失败: {e}")
            return self._create_error_result(
                error=str(e),
                duration=duration,
            )
...
def create_keyframe_to_video_generator(
    ...
    **kwargs
    ...
    return KeyframeToVideoGenerator(
        generator_id="keyframe_to_video",
        api_key=api_key,
        base_url=base_url,
        provider=provider,
        **kwargs
