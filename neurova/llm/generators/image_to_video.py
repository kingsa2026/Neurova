...
class ImageToVideoGenerator(BaseGenerator):
    ...
    def __init__(
        self,
        generator_id: str = "image_to_video",
        api_key: str = "",
        base_url: str = "",
        ...
        **kwargs,
        ...
        Args:
            ...
            **kwargs: 其他配置参数
        """
        super().__init__(generator_id=generator_id, generator_type=GeneratorType.IMAGE_TO_VIDEO, **kwargs)
        self.api_key = api_key
        self.base_url = base_url
        self.provider = provider
        self._initialized = True

        self.logger.info("ImageToVideoGenerator 初始化完成: provider=%s", provider)
    ...
    def supports(self, config: GenerationConfig) -> bool:
        ...
        """
        # 检查是否为图生视频类型
        if config.type != GeneratorType.IMAGE_TO_VIDEO:
            return False
    ...
    async def generate(self, config: GenerationConfig) -> GenerationResult:
        ...
        try:
            ...
            duration = time.time() - start_time
            self.logger.error("图生视频生成失败: %s", e)
            return self._create_error_result(
                error=str(e),
                duration=duration,
            )
    ...
    async def _generate_svd(self, config: GenerationConfig) -> GenerationResult:
        ...
        async with aiohttp.ClientSession() as session:
            # 提交任务
            async with session.post(
                f"{self.base_url}/v1/generation/image-to-video",
                json=request_data,
                headers=headers,
                timeout=aiohttp.ClientTimeout(total=30),
            ) as response:
                if response.status == 200:
                    ...
                    # 轮询任务状态
                    video_data = await self._poll_task_status(session, headers, task_id, "svd")
    ...
    async def _poll_task_status(
        ...
        # 根据服务商选择状态查询端点
        status_urls = {
            "kling": f"{self.base_url}/v1/videos/image2video/{task_id}",
            "runway": f"{self.base_url}/v1/tasks/{task_id}",
            "svd": f"{self.base_url}/v1/generation/image-to-video/{task_id}",
...
# 工厂函数
def create_image_to_video_generator(
    ...
    Returns:
        ImageToVideoGenerator 实例
    """
    return ImageToVideoGenerator(
        generator_id="image_to_video", api_key=api_key, base_url=base_url, provider=provider, **kwargs
    )
