...
class ImageToVideoGenerator(BaseGenerator):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super().__init__(
            ...
            generator_type=GeneratorType.IMAGE_TO_VIDEO,
            **kwargs
        )
        ...
        self.logger.info(f"ImageToVideoGenerator 初始化完成: provider={provider}")
    ...
    async def generate(self, config: GenerationConfig) -> GenerationResult:
        ...
        try:
            ...
            duration = time.time() - start_time
            self.logger.error(f"图生视频生成失败: {e}")
            return self._create_error_result(
                error=str(e),
                duration=duration,
            )
