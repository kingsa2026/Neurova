...
class NotificationManager:
    ...
    def add_notification(
        ...
        with self._lock:
            ...
            logger.info(f"添加通知: {notification_id}, user={user_id}, type={notification_type}")
            return notification
    ...
    def _schedule_negative_screen_push(
        ...
        try:
            ...
            if not config or not config.enabled or not config.auth_code:
                logger.debug(f"用户 {user_id} 未配置负一屏推送")
                return
                ...
                try:
                    ...
                    for callback in self._push_callbacks:
                        try:
                            ...
                            logger.error(f"推送回调执行失败: {e}")
                    
                    if result.success:
                        logger.info(f"负一屏推送成功: {notification.notification_id}")
                    else:
                        logger.warning(f"负一屏推送失败: {notification.notification_id}, error={result.error}")
                        
                except Exception as e:
                    logger.error(f"负一屏推送异常: {notification.notification_id}, error={e}")
            
            thread = threading.Thread(target=push_async, daemon=True)
            thread.start()
            
        except Exception as e:
            logger.error(f"调度负一屏推送失败: {e}")
