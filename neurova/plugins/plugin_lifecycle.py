...
class LifecycleHook:
    ...
    def execute(self):
        ...
        try:
            ...
            logger.error("Failed to execute lifecycle hook for %s: %s", self.plugin_name, e)
            raise
...
class PluginLifecycleManager:
    ...
    def register_hook(self, hook: LifecycleHook):
        ...
        logger.debug("Registered lifecycle hook: %s for %s", hook.event.value, hook.plugin_name)
    ...
    def unregister_hooks(self, plugin_name: str):
        ...
        logger.debug("Unregistered all hooks for plugin: %s", plugin_name)
    ...
    def execute_lifecycle(self, event: LifecycleEvent, plugin_name: str, **kwargs):
        ...
        for hook in hooks:
            try:
                ...
                logger.error("Failed to execute hook for %s on %s: %s", event.value, plugin_name, e)
                # 可以选择是否继续执行其他钩子
                continue
        ...
        logger.debug("Executed lifecycle event: %s for %s", event.value, plugin_name)
    ...
    def set_plugin_state(self, plugin_name: str, state: str):
        ...
        logger.debug("Set plugin state: %s -> %s", plugin_name, state)
    ...
    def add_event_listener(self, event: LifecycleEvent, callback: Callable):
        ...
        logger.debug("Added event listener for %s", event.value)
    ...
    def remove_event_listener(self, event: LifecycleEvent, callback: Callable):
        ...
        if event.value in self._event_listeners:
            try:
                ...
                logger.debug("Removed event listener for %s", event.value)
            except ValueError:
                logger.warning("Event listener not found for %s", event.value)
    ...
    def _trigger_event_listeners(self, event: LifecycleEvent, plugin_name: str, **kwargs):
        ...
        for listener in listeners:
            try:
                ...
                logger.error("Failed to trigger event listener for %s: %s", event.value, e)
    ...
    def clear_plugin_hooks(self, plugin_name: str):
        ...
        logger.info("Cleared hooks for plugin: %s", plugin_name)

    def get_hook_count(self, event: Optional[LifecycleEvent] = None, plugin_name: Optional[str] = None) -> int:
        """
        获取钩子数量
    ...
    def list_hooks(
        self, event: Optional[LifecycleEvent] = None, plugin_name: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        列出钩子
...
def register_lifecycle_hook(
    event: LifecycleEvent, callback: Callable, plugin_name: str = "", priority: int = 0, description: str = ""
) -> LifecycleHook:
    """
    注册生命周期钩子（便捷函数）
    ...
    """
    hook = LifecycleHook(
        event=event, callback=callback, priority=priority, plugin_name=plugin_name, description=description
    )
