...
class SkillPoolManager:
    ...
    def _init_dirs(self) -> None:
        ...
        logger.debug("Initialized skill pool directories: %s", self._base_dir)
    ...
    def list_public_skills(self) -> typing.List[SkillMetadata]:
        ...
        for skill_name, skill_data in self._metadata.get("public_skills", {}).items():
            try:
                ...
                logger.warning("Failed to load public skill %s: %s", skill_name, e)
        return skills
    ...
    def get_public_skill(self, name: str) -> typing.Optional[SkillMetadata]:
        ...
        if skill_data:
            try:
                ...
                logger.warning("Failed to load public skill %s: %s", name, e)
        return None
    ...
    def install_public_skill(self, skill_metadata: SkillMetadata) -> bool:
        ...
        try:
            ...
            logger.info("Installed public skill: %s", skill_metadata.name)
            return True

        except Exception as e:
            logger.error("Failed to install public skill %s: %s", skill_metadata.name, e)
            return False
    ...
    def _copy_public_skill_to_private(self, user_id: str, skill_name: str) -> bool:
        ...
        try:
            ...
            if not src_dir.exists():
                logger.warning("Public skill not found: %s", skill_name)
                return False
            ...
            logger.debug("Copied public skill %s to private pool for user %s", skill_name, user_id)
            return True

        except Exception as e:
            logger.error("Failed to copy public skill %s to private pool: %s", skill_name, e)
            return False
    ...
    def list_private_skills(self, user_id: str) -> typing.List[SkillMetadata]:
        ...
        for skill_name, skill_data in user_skills.items():
            try:
                ...
                logger.warning("Failed to load private skill %s for user %s: %s", skill_name, user_id, e)
    ...
    def create_private_skill(self, user_id: str, skill_metadata: SkillMetadata) -> bool:
        ...
        try:
            # 更新元数据
            self._metadata.setdefault("private_skills", {}).setdefault(user_id, {})[
                skill_metadata.name
            ] = skill_metadata.to_dict()
            self._save_metadata()
            ...
            logger.info("Created private skill %s for user %s", skill_metadata.name, user_id)
            return True

        except Exception as e:
            logger.error("Failed to create private skill %s for user %s: %s", skill_metadata.name, user_id, e)
            return False
    ...
    def update_private_skill(self, user_id: str, skill_metadata: SkillMetadata) -> bool:
        ...
        try:
            ...
            if skill_metadata.name not in self._metadata.get("private_skills", {}).get(user_id, {}):
                logger.warning("Private skill %s not found for user %s", skill_metadata.name, user_id)
                return False
            ...
            logger.info("Updated private skill %s for user %s", skill_metadata.name, user_id)
            return True

        except Exception as e:
            logger.error("Failed to update private skill %s for user %s: %s", skill_metadata.name, user_id, e)
            return False
    ...
    def delete_private_skill(self, user_id: str, skill_name: str) -> bool:
        ...
        try:
            ...
            logger.error("Failed to delete private skill %s for user %s: %s", skill_name, user_id, e)
            return False
    ...
    def share_private_skill(self, user_id: str, skill_name: str, target_user_id: str) -> bool:
        ...
        try:
            ...
            if not skill_metadata:
                logger.warning("Private skill %s not found for user %s", skill_name, user_id)
                return False
            ...
            self._metadata.setdefault("private_skills", {}).setdefault(target_user_id, {})[
                skill_name
            ] = skill_metadata.to_dict()

            self._save_metadata()

            logger.info("Shared skill %s from user %s to user %s", skill_name, user_id, target_user_id)
            return True

        except Exception as e:
            logger.error("Failed to share skill %s: %s", skill_name, e)
            return False
    ...
    def push_skill_to_agent(self, user_id: str, skill_name: str, agent_id: str) -> bool:
        ...
        try:
            ...
            logger.info("Pushed skill %s to agent %s", skill_name, agent_id)
            return True

        except Exception as e:
            logger.error("Failed to push skill %s to agent %s: %s", skill_name, agent_id, e)
            return False
    ...
    def unpush_skill_from_agent(self, agent_id: str, skill_name: str) -> bool:
        ...
        try:
            ...
            logger.info("Unpushed skill %s from agent %s", skill_name, agent_id)
            return True

        except Exception as e:
            logger.error("Failed to unpush skill %s from agent %s: %s", skill_name, agent_id, e)
            return False
    ...
    def get_agent_skills(self, agent_id: str) -> typing.List[SkillMetadata]:
        ...
        for skill_name, skill_data in agent_skills.items():
            try:
                ...
                logger.warning("Failed to load agent skill %s: %s", skill_name, e)
    ...
    def admin_delete_user_skills(self, user_id: str) -> bool:
        ...
        try:
            ...
            logger.info("Deleted all skills for user %s", user_id)
            return True

        except Exception as e:
            logger.error("Failed to delete skills for user %s: %s", user_id, e)
            return False
