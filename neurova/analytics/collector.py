...
class MetricsCollector:
    ...
    async def collect_task_metrics(self, task_id: str, task_type: str, agent_id: str, user_id: Optional[str] = None) -> TaskMetrics:
        """
        收集任务指标
    ...
    async def record_task(self, task_id: str, task_type: str, agent_id: str, user_id: Optional[str] = None) -> TaskMetrics:
        """
        记录任务（别名方法）
    ...
    async def update_task_status(self, task_id: str, status: TaskStatus, error_message: Optional[str] = None) -> Optional[TaskMetrics]:
        """
        更新任务状态
    ...
    async def record_realtime_metric(self, name: str, value: float, metric_type: MetricType, 
                                   unit: Optional[str] = None, tags: Optional[Dict[str, str]] = None) -> RealtimeMetric:
        """
        记录实时指标
    ...
    async def get_time_series(self, key: str, start_time: Optional[float] = None, 
        ...
        while True:
            try:
                ...
                update = {
                    ...
                    "active_users": len([m for m in self._user_metrics.values() if m.last_active_at and time.time() - m.last_active_at < 3600])
                }
    ...
    async def record_agent_request(self, agent_id: str, request_type: str, success: bool, 
                                 response_time: float, tokens_used: int = 0, cost: float = 0.0):
        """
        记录Agent请求
