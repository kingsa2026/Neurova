...
class InvitationCodeModel:
    ...
    def use_code(
        ...
        try:
            ...
            if not invitation_code.can_be_used():
                logger.warning(
                    "Invitation code cannot be used: %s (expired=%s, used_up=%s)",
                    code,
                    invitation_code.is_expired,
                    invitation_code.is_used_up,
                )
                return None
    ...
    def list_codes(
        self, include_expired: bool = False, include_inactive: bool = False, limit: int = 100, offset: int = 0
    ) -> List[InvitationCode]:
        """
        列出邀请码
