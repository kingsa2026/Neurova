...
@router.get("/grouped")
async def get_memories_grouped(group_by: str = Query("day", regex="^(day|week|month)$"), days: int = Query(30, ge=1, le=365)):
    """获取记忆时间线（按天/周/月分组）"""
    now = datetime.datetime.utcnow()
    groups = []
    for i in range(min(days, 30)):
        d = now - datetime.timedelta(days=i)
        key = d.strftime("%Y-%m-%d") if group_by == "day" else d.strftime("%Y-W%W") if group_by == "week" else d.strftime("%Y-%m")
...
async def get_timeline_stats():
    ...
    return {
        "code": 0, "message": "success",
        "data": {
            "total_memories": 0, "memories_today": 0, "memories_this_week": 0, "memories_this_month": 0,
            "most_active_day": None, "avg_per_day": 0.0,
        },
    }
