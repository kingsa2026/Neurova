...
async def create_log(body: LogCreate):
    ...
    if wls and hasattr(wls, "create_log"):
        try:
            ...
            logger.warning(f"WorkLogSystem.create_log failed: {e}")
...
async def list_logs(
    ...
    if wls and hasattr(wls, "list_logs"):
        try:
            ...
            logger.warning(f"WorkLogSystem.list_logs failed: {e}")
...
async def export_logs(
    ...
    if start_date:
        logs = [l for l in logs if datetime.datetime.fromtimestamp(l.get("created_at", 0)).date().isoformat() >= start_date]
    if end_date:
        logs = [l for l in logs if datetime.datetime.fromtimestamp(l.get("created_at", 0)).date().isoformat() <= end_date]
