...
_AGENT_ID_PATTERN = __import__("re").compile(r"^[a-zA-Z0-9_-]{1,64}$")
...
def _workspace_root(agent_id: str) -> Path:
    ...
    root = Path("agent_workspaces") / agent_id
    root.mkdir(parents=True, exist_ok=True)
    return root.resolve()


def _resolve_inside(root: Path, rel_path: str) -> Path:
    """把 rel_path 解析到 root 内；逃逸一律 400（fail-closed）。"""
...
async def download_workspace_zip(
    ...
    buf.seek(0)
    name = f"{agent_id}_{(subdir or 'workspace').replace('/', '_')}.zip"
