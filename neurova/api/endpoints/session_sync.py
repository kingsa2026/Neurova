...
async def websocket_sync(websocket: WebSocket, session_id: str,
    ...
    try:
        ...
        logger.info(f"WebSocket disconnected: {ws_conn.connection_id}")
    except Exception as e:
        ...
        if session_id in _ws_connections:
            _ws_connections[session_id] = [
                c for c in _ws_connections[session_id]
                if c.connection_id != ws_conn.connection_id
            ]
