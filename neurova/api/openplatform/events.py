...
class WebhookDeliveryJob:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status >= 200 and response.status < 300:
                        ...
                        logger.warning("Webhook delivery failed: %s (HTTP %d)", 
                                     self.delivery_id, response.status)
    ...
    async def _schedule_retry(self):
        ...
        if self.delivery.should_retry():
            logger.info("Scheduling retry for delivery %s (attempt %d)", 
                       self.delivery_id, self.delivery.attempts + 1)
            # 延迟后重试
