...
from fastapi import APIRouter, HTTPException, Header, Path as FPath, Query, Request
from pydantic import BaseModel

from neurova.api.openplatform.events import EventSystem, EventTypes, Event, get_event_system, WebhookDeliveryJob
from neurova.api.openplatform.models import (
    ApiKey, ApiKeyCreate, ApiScope, App, AppCreate, AppType, AppUpdate,
    DeliveryStatus, WebhookCreate, WebhookEndpoint, WebhookEventType, WebhookUpdate,
    generate_app_id, generate_key_id, generate_api_key, generate_webhook_id, hash_api_key,
)
...
async def list_apps(request: Request, page: int = Query(1, ge=1), page_size: int = Query(20, ge=1, le=100)):
    ...
    return {"apps": [App.from_dict(a) for a in apps[start:start + page_size]], "total": len(apps), "page": page, "page_size": page_size}

@router.post("/apps", response_model=App)
async def create_app(request: Request, body: AppCreate):
    uid = _get_user_id(request)
    app = App(app_id=generate_app_id(), app_name=body.app_name, app_type=body.app_type,
              description=body.description, website=body.website, logo_url=body.logo_url,
              redirect_uris=body.redirect_uris, scopes=body.scopes, owner_id=uid, metadata=body.metadata)
    data = _load_data_file("apps.json"); data.setdefault("apps", []).append(app.to_dict())
...
async def update_app(request: Request, app_id: str = FPath(...), body: AppUpdate = ...):
    uid = _get_user_id(request); _verify_app_owner(app_id, uid)
    data = _load_data_file("apps.json")
    for i, a in enumerate(data.get("apps", [])):
        if a.get("app_id") == app_id:
            ...
            data["apps"][i] = a; _save_data_file("apps.json", data)
...
async def delete_app(request: Request, app_id: str = FPath(...)):
    uid = _get_user_id(request); _verify_app_owner(app_id, uid)
    data = _load_data_file("apps.json"); apps = data.get("apps", [])
...
async def create_webhook(request: Request, app_id: str = FPath(...), body: WebhookCreate = ...):
    uid = _get_user_id(request); _verify_app_owner(app_id, uid)
    wh = WebhookEndpoint(webhook_id=generate_webhook_id(), app_id=app_id, url=body.url,
                         events=body.events, description=body.description, metadata=body.metadata)
    data = _load_data_file("webhooks.json"); data.setdefault("webhooks", []).append(wh.to_dict())
...
async def update_webhook(request: Request, app_id: str = FPath(...), webhook_id: str = FPath(...), body: WebhookUpdate = ...):
    uid = _get_user_id(request); _verify_app_owner(app_id, uid)
    ...
    for i, w in enumerate(data.get("webhooks", [])):
        if w.get("webhook_id") == webhook_id and w.get("app_id") == app_id:
            ...
            w["updated_at"] = time.time(); data["webhooks"][i] = w
            _save_data_file("webhooks.json", data)
            return WebhookEndpoint.from_dict(w)
    raise HTTPException(404, "Webhook不存在")
...
async def delete_webhook(request: Request, app_id: str = FPath(...), webhook_id: str = FPath(...)):
    uid = _get_user_id(request); _verify_app_owner(app_id, uid)
    data = _load_data_file("webhooks.json"); whs = data.get("webhooks", [])
...
async def list_webhook_deliveries(request: Request, app_id: str = FPath(...), webhook_id: str = FPath(...),
...
async def test_webhook(request: Request, app_id: str = FPath(...), webhook_id: str = FPath(...)):
    ...
    if endpoint is None: raise HTTPException(404, "Webhook不存在")
    test_event = Event(event_id=f"test_{secrets.token_hex(8)}", event_type=EventTypes.CUSTOM,
                       payload={"test": True, "message": "这是一条测试事件", "timestamp": time.time()}, source="test")
    job = WebhookDeliveryJob(test_event, endpoint)
    delivery = await job.execute()
    return {"message": "测试事件发送完成", "delivery_id": delivery.delivery_id,
            "status": delivery.status.value, "response_status": delivery.response_status,
...
async def create_key(request: Request, app_id: str = FPath(...), body: ApiKeyCreate = ...):
    uid = _get_user_id(request); _verify_app_owner(app_id, uid)
    api_key = generate_api_key(); kid = generate_key_id()
    expires_at = time.time() + body.expires_in if body.expires_in else None
    obj = ApiKey(key_id=kid, key_hash=hash_api_key(api_key), app_id=app_id, name=body.name,
                 scopes=body.scopes, expires_at=expires_at, rate_limit=body.rate_limit, metadata=body.metadata)
    data = _load_data_file("api_keys.json"); data.setdefault("keys", []).append(obj.to_dict())
    _save_data_file("api_keys.json", data)
    return {"key_id": kid, "api_key": api_key, "name": body.name,
            "scopes": [s.value for s in body.scopes], "expires_at": expires_at, "created_at": obj.created_at}

@router.delete("/apps/{app_id}/keys/{key_id}")
async def revoke_key(request: Request, app_id: str = FPath(...), key_id: str = FPath(...)):
    uid = _get_user_id(request); _verify_app_owner(app_id, uid)
    data = _load_data_file("api_keys.json"); found = False
    for k in data.get("keys", []):
        if k.get("key_id") == key_id and k.get("app_id") == app_id:
            k["is_active"] = False; k["revoked_at"] = time.time(); found = True; break
    if not found: raise HTTPException(404, "API密钥不存在")
    _save_data_file("api_keys.json", data)
...
async def get_docs_index():
    return {
        "title": "Neurova 开放平台 API 文档", "version": "1.0.0",
        "description": "Neurova 开放平台API，用于第三方应用集成。",
        "sections": [
            {"title": "应用管理", "endpoints": [
                {"method": "GET", "path": "/openplatform/apps"}, {"method": "POST", "path": "/openplatform/apps"},
...
def _get_scope_description(scope: ApiScope) -> str:
    _map = {
        ApiScope.USER_READ: "读取用户信息", ApiScope.USER_WRITE: "修改用户信息", ApiScope.USER_ADMIN: "用户管理",
        ApiScope.AGENT_READ: "读取Agent信息", ApiScope.AGENT_WRITE: "修改Agent配置", ApiScope.AGENT_ADMIN: "Agent管理",
        ApiScope.MEMORY_READ: "读取记忆", ApiScope.MEMORY_WRITE: "写入记忆", ApiScope.MEMORY_ADMIN: "记忆管理",
        ApiScope.TOOL_READ: "读取工具信息", ApiScope.TOOL_WRITE: "修改工具配置", ApiScope.TOOL_ADMIN: "工具管理",
        ApiScope.WEBHOOK_READ: "读取Webhook", ApiScope.WEBHOOK_WRITE: "修改Webhook", ApiScope.WEBHOOK_ADMIN: "Webhook管理",
        ApiScope.API_KEY_READ: "读取API密钥", ApiScope.API_KEY_WRITE: "修改API密钥", ApiScope.API_KEY_ADMIN: "API密钥管理",
        ApiScope.SYSTEM_READ: "读取系统信息", ApiScope.SYSTEM_WRITE: "修改系统配置", ApiScope.SYSTEM_ADMIN: "系统管理",
        ApiScope.ANALYTICS_READ: "读取分析数据", ApiScope.ANALYTICS_WRITE: "写入分析数据",
        ApiScope.ALL: "全部权限",
    }
...
async def get_stats(request: Request):
    ...
    return {
        "user_id": uid, "total_apps": len(apps), "active_apps": sum(1 for a in apps if a.get("is_active")),
        "total_api_keys": len(keys), "active_api_keys": sum(1 for k in keys if k.get("is_active")),
        "total_webhooks": len(whs), "active_webhooks": sum(1 for w in whs if w.get("is_active")),
