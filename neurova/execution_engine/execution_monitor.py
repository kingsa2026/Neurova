...
class ExecutionMonitor:
    ...
    def record_metric(
        self, name: str, value: float, metric_type: MetricType = MetricType.GAUGE, tags: typing.Dict[str, str] = None
    ) -> None:
        """记录指标"""
    ...
    def create_alert(self, level: AlertLevel, title: str, message: str, source: str = "") -> AlertRecord:
        ...
        logger.warning("告警: [%s] %s: %s", level.value, title, message)
    ...
    def start_trace(self, execution_id: str, task_name: str = "", agent_id: str = "") -> ExecutionTrace:
        """开始执行轨迹"""
        trace = ExecutionTrace(execution_id=execution_id, task_name=task_name, agent_id=agent_id)
        with self._lock:
    ...
    def add_trace_log(self, execution_id: str, message: str, level: str = "info") -> None:
        ...
        if trace:
            trace.metadata.setdefault("logs", []).append(
                {"message": message, "level": level, "timestamp": datetime.datetime.now().isoformat()}
            )
    ...
    def get_alerts(
        self, level: AlertLevel = None, acknowledged: bool = None, limit: int = 100
    ) -> typing.List[AlertRecord]:
        """获取告警"""
        alerts = self._alerts
        if level:
    ...
    def record_step(
        self, execution_id: str, step_name: str, status: str = "running", input_data: typing.Any = None
    ) -> typing.Optional[ExecutionStep]:
        """记录执行步骤"""
        trace = self._traces.get(execution_id)
        if not trace:
            return None

        step = ExecutionStep(name=step_name, status=status, start_time=datetime.datetime.now(), input_data=input_data)
    ...
    def _cleanup_old_traces(self, max_age_days: int = 30) -> int:
        ...
        with self._lock:
            ...
            self._execution_history = collections.deque(
                (t for t in self._execution_history if t.start_time > cutoff), maxlen=1000
            )
