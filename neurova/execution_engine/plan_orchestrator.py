...
class PlanOrchestrator:
    ...
    def register_step_executor(self, step_type: str, 
        ...
        logger.info(f"Registered step executor for type: {step_type}")
    ...
    async def execute_plan(self, plan_id: str, 
        ...
        logger.info(f"Executing plan: {plan.name} (ID: {plan.id})")
        ...
        try:
            ...
            if plan.is_complete():
                ...
                logger.warning(f"Plan failed: {plan.name}")
    ...
    async def _execute_steps(self, plan: ExecutionPlan, 
    ...
    def _get_executable_steps(self, plan: ExecutionPlan, 
                             dependency_graph: Dict[str, List[str]]) -> List[ExecutionStep]:
        """获取可执行的步骤"""
        executable = []
        ...
        for step in plan.steps:
            ...
            dependencies_met = all(
                plan.get_step(dep_id) and 
                plan.get_step(dep_id).status in [StepStatus.COMPLETED, StepStatus.SKIPPED]
    ...
    async def _execute_step_with_retry(self, step: ExecutionStep, 
        ...
        for attempt in range(step.max_retries + 1):
            try:
                ...
                logger.info(f"Step completed: {step.name} (attempt {attempt + 1})")
                return
                ...
                if attempt < step.max_retries:
                    logger.warning(f"Step failed, retrying: {step.name} (attempt {attempt + 1})")
    ...
    def _analyze_complexity(self, task_description: str, 
                           context: Optional[Dict[str, Any]] = None) -> TaskComplexity:
        """
        分析任务复杂度
    ...
    def _optimize_plan(self, plan: ExecutionPlan) -> ExecutionPlan:
        ...
        logger.info(f"Optimized plan: {len(parallel_groups)} parallel groups identified")
        return plan
    
    def _identify_parallel_groups(self, plan: ExecutionPlan, 
