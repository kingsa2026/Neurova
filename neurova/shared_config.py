...
class SharedConfigManager:
    ...
    def __init__(self, config_path: Path = None):
        ...
        logger.info(f"SharedConfigManager 初始化完成，配置文件: {self._config_path}")
        ...
        with self._lock:
            ...
            for existing in providers:
                if existing.get("id") == provider.get("id"):
                    logger.warning(f"提供商已存在: {provider.get('id')}")
                    return False
            ...
            logger.info(f"添加 LLM 提供商: {provider.get('id')}")
            return True
        ...
        with self._lock:
            ...
            for i, provider in enumerate(providers):
                if provider.get("id") == provider_id:
                    ...
                    logger.info(f"移除 LLM 提供商: {provider_id}")
                    return True
        ...
        with self._lock:
            ...
            for existing in servers:
                if existing.get("id") == server.get("id"):
                    logger.warning(f"服务器已存在: {server.get('id')}")
                    return False
            ...
            logger.info(f"添加 MCP 服务器: {server.get('id')}")
            return True
        ...
        with self._lock:
            ...
            for i, server in enumerate(servers):
                if server.get("id") == server_id:
                    ...
                    logger.info(f"更新 MCP 服务器: {server_id}")
                    return True
