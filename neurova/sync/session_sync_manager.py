...
class UnifiedSession:
    ...
    def register_channel(self, channel_type: str, send_callback: Callable, 
                         metadata: Optional[Dict[str, Any]] = None) -> ChannelConnection:
        """注册渠道连接"""
...
class SessionSyncManager:
    ...
    def create_session(self, user_id: str, agent_id: str = "default",
                       external_id: Optional[str] = None,
                       metadata: Optional[Dict[str, Any]] = None) -> UnifiedSession:
        """
        创建或获取会话
        ...
        with self._lock:
            ...
            logger.info(f"Created new session: {session.session_id} for user={user_id}, agent={agent_id}")
            return session
    ...
    def list_sessions(self, user_id: Optional[str] = None, 
                      ...
                      status: Optional[str] = None) -> List[UnifiedSession]:
        """列出会话"""
    ...
    def register_channel(self, session_id: str, channel_type: str,
                         send_callback: Callable,
                         metadata: Optional[Dict[str, Any]] = None) -> Optional[ChannelConnection]:
        """
        注册渠道连接
        ...
        with self._lock:
            ...
            logger.info(f"Registered channel {channel_type} to session {session_id}")
            return conn
    ...
    def unregister_channel(self, session_id: str, channel_type: str) -> bool:
        ...
        with self._lock:
            ...
            if session.unregister_channel(channel_type):
                # 发送渠道断开事件
                event = SessionEvent(
                    event_type=EventType.CHANNEL_DISCONNECTED,
                    session_id=session_id,
                    source_channel=channel_type
                )
                session.add_event(event)
                
                logger.info(f"Unregistered channel {channel_type} from session {session_id}")
                return True
    ...
    async def broadcast_event(self, session_id: str, event: SessionEvent,
                              exclude_channel: Optional[str] = None) -> int:
        """
        广播事件到所有活跃渠道
        ...
        logger.debug(f"Broadcast event {event.event_type} to {sent_count} channels in session {session_id}")
        return sent_count
    ...
    async def _send_to_channel(self, conn: ChannelConnection, event: SessionEvent) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to send to channel {conn.channel_type}: {e}")
            return False
    
    def broadcast_event_sync(self, session_id: str, event: SessionEvent,
                             exclude_channel: Optional[str] = None) -> int:
        """
        同步广播事件（用于非异步上下文）
    ...
    def get_history(self, session_id: str, limit: int = 100,
                    event_types: Optional[List[EventType]] = None) -> List[SessionEvent]:
        """获取会话历史"""
        session = self._sessions.get(session_id)
        if not session:
            return []
    ...
    def get_history_by_user(self, user_id: str, agent_id: str = "default",
