...
class LiteOCRProcessor:
    ...
    def _detect_pytesseract(self, image_data: bytes, width: int, height: int) -> typing.List[typing.Dict[str, typing.Any]]:
        """使用 pytesseract 检测"""
        import pytesseract
        ...
        try:
            ...
            if circles is not None:
                for circle in circles[0]:
                    cx, cy, r = circle
                    icons.append(BoundingBox(
                        x=cx - r, y=cy - r, width=2 * r, height=2 * r,
                        confidence=0.5, label="icon"
                    ))
...
class LiteBoxAnnotator:
    ...
    def annotate(self, image_data: bytes, width: int, height: int, 
                 boxes: typing.List[BoundingBox]) -> bytes:
        """在图像上绘制边界框"""
        if not CV2_AVAILABLE or not NUMPY_AVAILABLE:
            return image_data
