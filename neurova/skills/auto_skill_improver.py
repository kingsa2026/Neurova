...
class AutoSkillImprover:
    ...
    async def optimize_skill_prompt(self, skill_id: str, current_prompt: str) -> 'OptimizedPrompt':
        ...
        """
        from .prompt_optimizer import PromptOptimizer, OptimizationGoal
        
        # 创建提示优化器
        optimizer = PromptOptimizer()
        
        # 分析当前提示词
        analysis = await optimizer.analyze_prompt(current_prompt)
    ...
    async def run_prompt_ab_test(self, prompt_a: str, prompt_b: str, test_cases: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        运行 A/B 测试
