...
class _DangerousNodeVisitor(ast.NodeVisitor):
    ...
    DANGEROUS_BUILTINS = {
        ...
        'callable', 'repr', 'hash', 'id',
        'type', 'super', 'classmethod', 'staticmethod',
    }
    
    # 危险的 os 函数
    DANGEROUS_OS_FUNCTIONS = {
        'system', 'popen', 'exec', 'execle', 'execl',
        'execv', 'execve', 'execvp', 'execvpe',
        'spawn', 'spawnl', 'spawnle', 'spawnv', 'spawnve',
        'startfile', 'remove', 'unlink', 'rename',
        'rmdir', 'removedirs', 'makedirs', 'mkdir',
    }
        ...
        if not skill_path.exists():
            return SecurityReport(
                ...
                issues=[SecurityIssue(
                    level=SecurityLevel.CRITICAL,
                    description=f"Skill path does not exist: {skill_path}"
                )],
...
class SecurityManager:
    ...
    def execute_skill(self, skill_path: str, entry_point: str = "main.py") -> ExecutionResult:
        ...
        if report.has_critical_issues():
            return ExecutionResult(
                success=False,
                error="Skill has critical security issues",
                output=""
            )
    ...
    def set_security_policy(self, skill_name: str, policy: Dict[str, Any]):
        ...
        logger.info(f"Set security policy for {skill_name}")
    ...
    def remove_security_policy(self, skill_name: str):
        ...
        if skill_name in self._security_policies:
            ...
            logger.info(f"Removed security policy for {skill_name}")
    ...
    def batch_scan(self, skill_paths: List[str]) -> List[SecurityReport]:
        ...
        for skill_path in skill_paths:
            try:
                ...
                logger.error(f"Failed to scan {skill_path}: {e}")
