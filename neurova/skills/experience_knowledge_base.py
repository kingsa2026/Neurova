...
class ExperienceKnowledgeBase:
    ...
    def add_experience(
        ...
        self._save_to_disk()
        logger.debug("Added experience '%s' for skill '%s'", exp_id, skill_id)
        return exp
    ...
    def _save_to_disk(self) -> None:
        ...
        try:
            ...
            logger.warning("Failed to save experience KB: %s", e)
    ...
    def _load_from_disk(self) -> None:
        ...
        try:
            ...
            self._type_index = {ExperienceType(t): ids for t, ids in data.get("type_index", {}).items()}

            logger.info("Loaded %s experiences from disk", len(self._experiences))
        except Exception as e:
            logger.warning("Failed to load experience KB: %s", e)
