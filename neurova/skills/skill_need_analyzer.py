...
class SkillNeedAnalyzer:
    ...
    def analyze_and_acquire(self, request: str) -> List[SkillAcquisitionResult]:
        ...
        for skill_name in missing_skills:
            try:
                ...
                logger.error(f"Failed to acquire skill {skill_name}: {e}")
                results.append(SkillAcquisitionResult(
    ...
    def _acquire_skill(self, skill_name: str, context: str) -> SkillAcquisitionResult:
        ...
        if not search_results:
            return SkillAcquisitionResult(
                skill_name=skill_name,
                success=False,
                error="No matching skills found in markets"
            )
    ...
    def _select_best_match(self, search_results: List[SearchResult], 
                          skill_name: str, context: str) -> Optional[SearchResult]:
        """
        选择最佳匹配
    ...
    def _calculate_similarity(self, result: SearchResult, 
                            skill_name: str, context: str) -> float:
        """
        计算相似度
    ...
    def _install_skill(self, search_result: SearchResult) -> SkillAcquisitionResult:
        ...
        try:
            ...
            logger.info(f"Installing skill: {search_result.name} from {search_result.source}")
            ...
            logger.error(f"Failed to install skill {search_result.name}: {e}")
            return SkillAcquisitionResult(
                skill_name=search_result.name,
                success=False,
                source=search_result.source,
