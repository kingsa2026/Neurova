...
from .models import (
    ChainStatus, StepStatus, SkillChainStep, SkillChain,
    StepExecutionResult, ChainExecutionResult, ChainStatusInfo
)
...
class SkillChainExecutor:
    ...
    async def execute_chain(self, chain: SkillChain, initial_input: Dict[str, Any]) -> ChainExecutionResult:
        ...
        try:
            ...
            logger.info(f"开始执行技能链: {chain.name} ({chain_id})")
            ...
            for i, step in enumerate(chain.steps):
                ...
                # 更新输入数据
                current_input = await self._map_output_to_input(
            ...
            logger.info(f"技能链执行完成: {chain.name} ({chain_id})")
            return result
            
        except Exception as e:
            logger.error(f"技能链执行失败: {e}")
    ...
    async def resume_chain(self, chain_id: str) -> bool:
        ...
        if context["status"] != ChainStatus.PAUSED:
            logger.warning(f"技能链未处于暂停状态: {chain_id}")
            return False
        
        context["status"] = ChainStatus.RUNNING
        
        logger.info(f"技能链已恢复: {chain_id}")
        return True
    ...
    async def cancel_chain(self, chain_id: str) -> bool:
        ...
        logger.info(f"技能链已取消: {chain_id}")
        return True
    
    async def _execute_step(self, step: SkillChainStep, input_data: Dict[str, Any], context: Dict[str, Any]) -> StepExecutionResult:
        """
        执行单个步骤
        ...
        try:
            ...
            if self.skill_service:
                # 使用技能服务执行
                execution_result = await self.skill_service.call_skill(
                    skill_id=step.skill_id,
    ...
    async def _map_input_data(self, input_data: Dict[str, Any], mapping: Dict[str, str], context: Dict[str, Any]) -> Dict[str, Any]:
        """映射输入数据"""
        if not mapping:
            return input_data
    ...
    async def _map_output_to_input(self, output_data: Dict[str, Any], mapping: Dict[str, str], context: Dict[str, Any]) -> Dict[str, Any]:
        """将输出映射为下一步的输入"""
        if not mapping:
            return output_data
