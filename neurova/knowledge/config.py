        ...
        if _knowledge_config is None:
            ...
            if config_path and config_path.exists():
                try:
                    ...
                    logger.info(f"Loaded knowledge config from {config_path}")
...
def save_knowledge_config(config: KnowledgeBaseConfig, 
    ...
    with _config_lock:
        try:
            ...
            logger.info(f"Saved knowledge config to {config_path}")
            return True
...
def _find_config_file() -> Optional[Path]:
    ...
    for path in [current_dir / "knowledge_config.json", 
                 current_dir / "config" / "knowledge_config.json"]:
        if path.exists():
            return path
