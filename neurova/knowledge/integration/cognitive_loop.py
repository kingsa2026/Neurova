...
class CognitiveLoopCoordinator:
    ...
    def __init__(self, config: Optional[LoopConfig] = None):
        ...
        logger.info("CognitiveLoopCoordinator initialized: mode=%s", self.config.mode.value)
    ...
    def configure(
        ...
        logger.info(
            "CognitiveLoopCoordinator dependencies configured: " "memory=%s, knowledge=%s, evolution=%s",
    ...
    def trigger(self, trigger_type: TriggerType, data: Optional[Dict[str, Any]] = None) -> str:
        """
        触发闭环
        ...
        Returns:
            事件ID
        """
        event = LoopEvent(event_type=trigger_type.value, source="manual", data=data or {})

        # 添加到队列
        self._event_queue.put_nowait(event)

        logger.info("Triggered loop: %s, event_id=%s", trigger_type.value, event.event_id)
            ...
            try:
                ...
                try:
                    event = await asyncio.wait_for(
                        self._event_queue.get(), timeout=self.config.trigger_interval_seconds
                    )
                except asyncio.TimeoutError:
                    # 定时触发
                    if self.config.auto_trigger:
                        event = LoopEvent(event_type=TriggerType.SCHEDULED.value, source="auto")
                    else:
                        continue
                ...
                logger.error("Loop runner error: %s", e)
    ...
    async def _execute_loop(self, event: LoopEvent) -> None:
        ...
        try:
            ...
            if self.stats.avg_loop_duration_ms == 0:
                ...
                self.stats.avg_loop_duration_ms = self.stats.avg_loop_duration_ms * 0.9 + duration_ms * 0.1
            ...
            logger.info("Loop executed: %.2f, duration=%sms", event.event_type, duration_ms)

        except Exception as e:
            self.stats.failed_loops += 1
            self.stats.total_loops += 1

            logger.error("Loop execution failed: %s", e)
    ...
    async def _knowledge_to_memory(self, event: LoopEvent) -> None:
        ...
        if not topic:
            ...
            try:
                ...
                if recent:
                    topic = " ".join(str(item.get("content", ""))[:80] for item in recent[:2])
        ...
        if self._evolution_hub:
            try:
                ...
                if progress:
                    ...
                    reflection_parts.append(f"进化进度: 缺口总数={total_gaps}, 完成率={completion_rate * 100:.1f}%%")
            except Exception as exc:
    ...
    async def _evolution_feedback(self, event: LoopEvent) -> None:
        ...
        if self._memory_system:
            try:
                ...
                completion_rate = progress.get("completion_rate", 0)
                feedback_text = (
                    f"[进化反馈] 知识缺口: {total_gaps}, " f"已完成学习: {completed}, " f"完成率: {completion_rate * 100:.1f}%%"
                )
                await asyncio.to_thread(
                    self._memory_system.save_conversation_memory,
    ...
    async def _safe_callback(self, event_name: str, *args) -> None:
        ...
        for callback in self._callbacks.get(event_name, []):
            try:
                ...
                logger.error("Callback %s failed: %s", event_name, e)
...
def create_cognitive_loop(
    config: Optional[LoopConfig] = None, mode: Optional[LoopMode] = None
...
def create_scenario_loop(scenario: str) -> CognitiveLoopCoordinator:
    ...
    scenarios = {
        ...
        "full": LoopConfig(
            mode=LoopMode.FULL_LOOP, auto_trigger=True, trigger_interval_seconds=3600, evolution_enabled=True
        ),
    }
