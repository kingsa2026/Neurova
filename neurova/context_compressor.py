...
class SmartContextCompressor:
    ...
    def __init__(self, config: Optional[CompressionConfig] = None):
        ...
        logger.info(f"SmartContextCompressor 初始化，最大tokens: {self.config.max_tokens}")
    ...
    def compress_context(
        ...
        logger.info(f"开始压缩上下文，当前tokens: {total_tokens}，目标: {target}")
        ...
        logger.info(f"压缩完成，压缩后tokens: {self._count_tokens(compressed_memories + compressed_conversation)}")
    ...
    def _progressive_compress_history(
        self,
        messages: List[Message],
        budget_tokens: int
    ) -> Tuple[List[Message], str]:
    ...
    def _build_context(
        ...
        messages: List[Message],
        system_prompt: Optional[str] = None,
        memories: Optional[List[Message]] = None
    ) -> List[Dict[str, Any]]:
        """
        构建上下文
