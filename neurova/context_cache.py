...
class ContextCacheManager:
    ...
    def __init__(
        ...
        if enable_persistence:
            try:
                ...
                logger.warning("无法加载持久化引擎: %s", e)

        # 后台写入线程
        self._write_thread = None
        self._running = False

        logger.info("ContextCacheManager 初始化，最大条目: %s，最大内存: %sMB", max_entries, max_memory_mb)
        ...
        with self._lock:
            ...
            if self.enable_persistence:
                self._write_thread = threading.Thread(
                    target=self._background_write_loop, daemon=True, name="context-cache-writer"
                )
    ...
    def get_context(
        self, agent_id: str, channel: str, user_id: Optional[str] = None, session_id: Optional[str] = None
            ...
            if self._persistence and self.enable_persistence:
                try:
                    context_data = self._persistence.load_context(
                        agent_id=agent_id, channel=channel, user_id=user_id, session_id=session_id
                    )
                    ...
                    logger.warning("从持久化加载上下文失败: %s", e)

            return None

    def get_context_by_channel(self, agent_id: str, channel: str, limit: int = 100) -> List[Dict[str, Any]]:
        """
        按渠道获取上下文列表
    ...
    def get_context_with_agent(self, agent_id: str, limit: int = 100) -> List[Dict[str, Any]]:
        """
        获取指定Agent的所有上下文
    ...
    def batch_write(self) -> int:
        ...
        with self._lock:
            ...
            logger.info("批量写入完成，写入 %s 个条目", written_count)
    ...
    def flush_all(self) -> int:
        ...
        with self._lock:
            ...
            logger.info("刷新所有数据，写入 %s 个条目", written_count)
            return written_count
    ...
    def evict_if_needed(self) -> int:
        ...
        with self._lock:
            ...
            if evicted_count > 0:
                ...
                logger.info("淘汰 %s 个缓存条目", evicted_count)
    ...
    def evict_inactive(self, max_inactive_seconds: float = 3600.0) -> int:
        ...
        with self._lock:
            ...
            if evicted_count > 0:
                self._stats["evictions"] += evicted_count
                logger.info("淘汰 %s 个不活跃条目", evicted_count)
    ...
    def _write_to_disk(self, key: str) -> bool:
        ...
        try:
            ...
            entry.mark_clean()
            logger.debug("写入磁盘: %s", key)
            return True
        except Exception as e:
            logger.error("写入磁盘失败 %s: %s", key, e)
            return False
    ...
    def _background_write_loop(self):
        ...
        while self._running:
            try:
                ...
                if written > 0:
                    logger.debug("后台写入 %s 个条目", written)
            except Exception as e:
                logger.error("后台写入循环异常: %s", e)
