...
class ToolCache:
    ...
    def get(
        ...
        if key in self._l1_cache:
            ...
            if not entry.is_expired():
                ...
                logger.debug(f"L1 cache hit for {tool_name}")
                return entry.result
            else:
                # 过期，删除
        ...
        if l2_result is not None:
            ...
            logger.debug(f"L2 cache hit for {tool_name}")
            return l2_result
            ...
            for entry in self._l3_cache[tool_name]:
                if not entry.is_expired():
                    ...
                    if self._calculate_param_similarity(params, entry.params) > self._similarity_threshold:
                        ...
                        logger.debug(f"L3 cache hit for {tool_name}")
                        return entry.result
    ...
    def invalidate(
        ...
        if params is not None:
            ...
            if tool_name in self._l2_cache:
                self._l2_cache[tool_name] = [
                    (k, e) for k, e in self._l2_cache[tool_name]
                    if k != key
                ]
        else:
            # 使所有该工具的缓存失效
    ...
    def get_stats(self) -> typing.Dict[str, typing.Any]:
        ...
        return {
            ...
            "hit_rate": (
                ...
                max(1, self._stats["l1_hits"] + self._stats["l2_hits"] + self._stats["l3_hits"] + self._stats["misses"])
            )
        }
    ...
    def _calculate_param_similarity(
        self, 
        params1: typing.Dict[str, typing.Any], 
        params2: typing.Dict[str, typing.Any]
    ) -> float:
        """
        计算参数相似度
