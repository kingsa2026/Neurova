...
class VoiceEngine:
    ...
    def __init__(self, engine_type: VoiceEngineType, engine: Any):
        """
        初始化语音引擎
    ...
    async def process(self, input_data: Union[bytes, str], operation: str, **kwargs) -> VoiceResult:
        """统一处理接口
        ...
        try:
            ...
            self._logger.warning("语音处理失败: %s", e)
            return VoiceResult(error=str(e))

    async def _process_asr(self, audio_data: bytes, operation: str, **kwargs) -> VoiceResult:
        """处理 ASR 操作"""
        if operation == "transcribe":
            result = await self._engine.transcribe(audio_data, **kwargs)
            return VoiceResult(text=result.get("text"), confidence=result.get("confidence"), metadata=result)
        elif operation == "understand":
            result = await self._engine.understand(audio_data, **kwargs)
            return VoiceResult(metadata=result)
        elif operation == "caption":
            result = await self._engine.caption(audio_data, **kwargs)
            return VoiceResult(text=result, metadata={"operation": "caption"})
        else:
            return VoiceResult(error=f"不支持的 ASR 操作: {operation}")

    async def _process_tts(self, text: str, operation: str, **kwargs) -> VoiceResult:
        """处理 TTS 操作"""
        if operation == "synthesize":
            audio_data = await self._engine.synthesize(text, **kwargs)
            return VoiceResult(audio_data=audio_data, metadata={"operation": "synthesize"})
        else:
...
class VoiceEngineFactory:
    ...
    @staticmethod
    def create_asr_engine(engine_class: type, **kwargs) -> VoiceEngine:
        """创建 ASR 引擎"""
        engine = engine_class(**kwargs)
        return VoiceEngine(engine_type=VoiceEngineType.ASR, engine=engine)

    @staticmethod
    def create_tts_engine(engine_class: type, **kwargs) -> VoiceEngine:
        """创建 TTS 引擎"""
        engine = engine_class(**kwargs)
        return VoiceEngine(engine_type=VoiceEngineType.TTS, engine=engine)
...
class AutoVoiceEngine:
    ...
    def _select_first_available(self):
        ...
        for i, engine in enumerate(self._engines):
            if getattr(engine, "is_initialized", False):
                ...
                self._logger.info("自动选择引擎: %s (索引 %s)", engine.__class__.__name__, i)
                return
    ...
    def get_info(self) -> Dict[str, Any]:
        """获取引擎信息"""
        available = sum(1 for e in self._engines if getattr(e, "is_initialized", False))
        return {
            "engine_type": self._engine_type.value,
            "is_initialized": self.is_available(),
        ...
        for i in range(start_index, len(self._engines)):
            ...
            try:
                ...
                if self._is_result_valid(result):
                    # 成功：如果故障转移了，记录日志
                    if i != start_index:
                        self._logger.info("故障转移成功: %s (索引 %s)", engine.__class__.__name__, i)
                        self._current_index = i
                    return result

                # 结果无效（如空数据），尝试下一个引擎
                self._logger.warning("引擎 %s 返回无效结果，尝试下一个", engine.__class__.__name__)

            except Exception as e:
                self._logger.warning("引擎 %s 抛出异常: %s，尝试下一个", engine.__class__.__name__, e)
                continue

        return VoiceResult(error="所有语音引擎均处理失败")

    async def _process_with_engine(
        self, engine: Any, operation: str, input_data: Union[bytes, str], **kwargs
    ) -> VoiceResult:
        """使用指定引擎处理请求"""
