...
class ShutdownGuard:
    ...
    def write_sentinel(self, status: str = "running") -> None:
        ...
        with self._lock:
            try:
                ...
                logger.debug(f"Sentinel written: status={status}")
            except Exception as e:
                logger.warning(f"Failed to write sentinel: {e}")
    ...
    def check_abnormal_shutdown(self) -> bool:
        ...
        try:
            ...
            if status == "running":
                ...
                logger.warning(
                    f"Abnormal shutdown detected! Last run: PID={last_pid}, "
                    f"time={last_time}, status={status}"
                )
                return True
                ...
                logger.warning(f"Unknown sentinel status: {status}")
                return True
                
        except (json.JSONDecodeError, KeyError) as e:
            logger.warning(f"Corrupted sentinel file: {e}")
            return True
    ...
    def register_agent_buffer(self, agent_id: str, buffer: Any) -> None:
        ...
        with self._lock:
            ...
            logger.debug(f"Registered buffer for agent '{agent_id}'")
        ...
        for agent_id, buffer in buffers.items():
            try:
                ...
                if hasattr(buffer, 'flush'):
                    ...
                    logger.info(f"Flushed buffer for agent '{agent_id}'")
                elif hasattr(buffer, 'flush_to_storage'):
                    buffer.flush_to_storage()
                    results[agent_id] = True
                    logger.info(f"Flushed buffer for agent '{agent_id}'")
                else:
                    logger.warning(f"Buffer for agent '{agent_id}' has no flush method")
                    results[agent_id] = False
            except Exception as e:
                logger.error(f"Failed to flush buffer for agent '{agent_id}': {e}")
                results[agent_id] = False
    ...
    def _recover_agent_sessions(self, agent_id: str, agent_dir: Path) -> int:
        ...
        for session_file in agent_dir.glob("*.json"):
            try:
                ...
                # 这里应该将数据写入到记忆系统
                # 具体实现取决于记忆管理器的接口
                logger.info(f"Recovered session data from {session_file.name}")
                recovered += 1
                
            except (json.JSONDecodeError, KeyError) as e:
                logger.warning(f"Failed to parse session file {session_file}: {e}")
            except Exception as e:
                logger.warning(f"Failed to recover session {session_file}: {e}")
    ...
    def graceful_shutdown(self, timeout: float = 30.0) -> bool:
        ...
        logger.info(f"Buffer flush completed in {flush_time:.2f}s: {flush_results}")
        ...
        if elapsed > timeout:
            logger.warning(f"Shutdown took {elapsed:.2f}s (timeout: {timeout}s)")
            return False
    ...
    def prepare_startup(self) -> Dict[str, Any]:
        ...
        if self.check_abnormal_shutdown():
            ...
            if recovery:
                logger.info(f"Recovery completed: {recovery}")
            else:
