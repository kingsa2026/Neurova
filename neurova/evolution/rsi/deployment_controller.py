...
class RSIDeploymentController:
    ...
    def __init__(self, initial_phase: int = 0):
        ...
        logger.info(f"RSIDeploymentController initialized at phase {initial_phase}")
    ...
    def evaluate_phase_transition(self, metrics: Dict[str, Any]) -> bool:
        ...
        if days_without_rollback < required_days.get(self._current_phase, 0):
            logger.info(f"Not enough days without rollback: {days_without_rollback} < {required_days.get(self._current_phase, 0)}")
            return False
    ...
    def advance_phase(self) -> int:
        ...
        logger.info(f"Advanced to phase {self._current_phase}: {self._phase_descriptions[self._current_phase]}")
        return self._current_phase
