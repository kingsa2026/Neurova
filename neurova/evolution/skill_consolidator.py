...
对位 Hermes `agent/curator.py` 的 LLM 巩固 fork,但按 Neurova 的 C10 评审闸
哲学收窄为两段:
...
def find_prefix_clusters(names: list[str], min_size: int = _DEFAULT_MIN_CLUSTER) -> list[list[str]]:
    ...
    例:hermes-config-a / hermes-config-b / hermes-config-c →
    [["hermes-config-a", "hermes-config-b", "hermes-config-c"]]
    """
